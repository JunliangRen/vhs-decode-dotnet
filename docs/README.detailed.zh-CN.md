...
配对墙钟中位数从 30.213 降至 29.576 秒（缩短 1.35%），进程 CPU 时间从 290.719
降至 279.297 秒（缩短 3.93%），基线/候选峰值工作集为 425.4/388.6 MiB。另行三组
显式零线程配对的墙钟中位数从 37.492 降至 36.962 秒（缩短 0.43%），高线程收益没有
引入可测的低线程回退。
