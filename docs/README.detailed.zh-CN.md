...
三次，共 90 次 Release 运行。将严格独立的 CTI 扫描线上限从 5 调整为 8，并将
有宽度上限的 VHS 初始同步边沿扫描并行化后，10 和 20 worker 的四个 `current`
单元格各以十组交替基线/候选配对重新测量，另增加 80 次 Release 运行。Python v0.4.0 commit 为
