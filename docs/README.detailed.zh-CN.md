...
当前本机矩阵于 2026-08-20 使用源提交
`42674ca15c1bdcd35acd04758ab8595f35e28c5c` 测得。被测 `decode.exe` 的文件版本为
2.4.0.0，SHA-256 为
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 完整 CUDA 40 MSPS | 62.90（62.36-63.43） | 2.72 核 / 13.62% | 38.84% / 52.09% | 37.01% / 65% | 16.96% / 30% | 0% / 0% | 7,076 MiB（+3,935） |
| 完整 IPP 40 MSPS | 24.74（24.67-24.81） | 4.63 核 / 23.16% | 29.00% / 38.85% | 0.08% / 3% | 0.03% / 1% | 0% / 0% | 3,141 MiB（+0） |
| 完整 CUDA 20 MSPS | 69.72（69.51-69.93） | 2.83 核 / 14.13% | 41.42% / 49.60% | 27.38% / 58% | 10.42% / 23% | 0% / 0% | 5,328 MiB（+2,187） |
| 完整 IPP 20 MSPS | 32.89（32.60-33.17） | 5.46 核 / 27.28% | 44.24% / 62.69% | 0.04% / 1% | 0% / 0% | 0% / 0% | 3,141 MiB（+0） |
| 预览 CUDA 20 MSPS | 84.19（83.04-85.33） | 1.10 核 / 5.52% | 30.36% / 36.76% | 41.99% / 63% | 12.05% / 19% | 5.70% / 14% | 4,835 MiB（+1,694） |
| 预览 IPP 20 MSPS | 33.00（32.91-33.09） | 4.69 核 / 23.44% | 36.15% / 50.61% | 1.25% / 11% | 0.07% / 1% | 0.48% / 11% | 3,323 MiB（+182） |

紧邻测试前的 10 秒空闲基线平均为整机 CPU 7.32%、GPU SM 0.32%、NVENC 0%、
GPU 显存 3,141 MiB；表中的整机均值是原始值，没有减去这条有噪声的基线。CUDA 在
完整 40 MSPS、完整 20 MSPS 与 20 MSPS preview 中的源帧吞吐分别为 IPP 的
2.542x、2.120x、2.551x。在这份较长门禁中，完整解码从 40 降到 20 MSPS 后，CUDA
吞吐提高 10.85%，IPP 提高 32.94%。冷启动 W5 平均为 CUDA 0.722 秒、IPP 2.028 秒。
完整解码不编码视频，所以 NVENC 为零；CUDA preview 使用 block-linear NV12/NVENC，
IPP preview 则选择 `NVENC + CUDA YADIF x2`。六种配置各自在两轮中都只产生一套可重复
哈希。这些结果只对应本采集、本机、当前驱动与构建，不是跨硬件或画质等价保证。原始
CSV、日志与 100 ms 采样保存在本机
...
在本机 RTX 4070 和一份真实的 631.452 秒 PAL 40 MSPS 采集上，同一最终版本的
可执行文件为每个后端各启动两次，并分别请求 W15/W25/W30/W35，因而每个后端都有八个
稳态 2 秒观测。CUDA 平均 0.588094 秒，默认 20-thread IPP 预览为 1.479124 秒：墙钟
降低 60.24%，源帧吞吐达到 2.515 倍（85.020 对 33.804 fps）。`decode.exe` 进程 CPU
时间平均为 0.701172 对 8.156250 秒，但该指标不含各自的 FFmpeg 子进程。冷启动 W5
平均为 CUDA 1.592767 秒、IPP 1.829743 秒。两边输出均为 H.264 Main、768x576、
逐行 50 fps，并带预期 PAL 色彩标记。CUDA 起点约早一输出场；对齐这 20 ms 后，
W5/W15/W25/W30/W35 的平均渲染 SSIM Y/U/V/All 为
0.916844/0.957443/0.966783/0.931934。采用相同同步策略但关闭跨场 dropout 和色度稳定时
为 0.906637/0.952453/0.964071/0.923845，五个窗口的综合 SSIM 全部改善。作为包含运动
影响的闪烁代理指标，连续帧色度平均绝对差 U/V 分别降低 23.74%/16.66%，向 IPP 靠近；
这不是纯静态区域的闪烁指标。另一组 A-B-B-A 中，两项输出处理启用前后分别为
0.597190/0.604505 秒，即墙钟代价 1.23%、吞吐降低 1.21%。另一组五窗口 CUDA 12 场
对 16 场比较的平均 SSIM 为 0.9716，最低为 0.960934，且没有一帧 cadence 偏移。这是
预览画质证据，不代表 Exact 一致或跨 GPU 保证；虽然合成 NTSC 原生管线已通过，真实
NTSC 采集画质仍未认证。
...
该后端采用独立数值契约，不声明兼容 `v0.4.0` 或 `current` 的哈希。较早的优化前
...
普遍速度保证。保留这组数据只是为了追溯历史；当前吞吐应以上面的资源矩阵为准。
...
一套 hash，全部 1,500 项标准 xUnit v3 测试通过。
...
2026-08-15 用 commit `21b8b01` 构建的 .NET 11 Preview 7 自包含候选二进制同时刷新。
本次文档/工具链刷新不发布新标签或 Release。每个 .NET 单元格依次给出墙钟中位数、
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 52.811 s | 54.243 s | 13.977 s / 3.778x / 73.53% | 12.556 s / 4.320x / 76.85% | 12.214 s / 4.324x / 76.87% | 9.419 s / 5.759x / 82.64% |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 52.583-62.222 s | 53.893-58.195 s | 13.935-14.207 s | 12.236-13.228 s | 12.100-13.038 s | 9.255-10.015 s |
...
同一个 .NET 11 Preview 7 候选二进制在 2026-08-15 完成的 60 次运行。每个三次运行
...
全部刷新的 .NET 单元格都使用 commit `21b8b01` 构建的 Preview 7 自包含候选二进制；
其 product version 为 `2.1.0+21b8b01998fb7519cf3616820e181dba93f23d10`，单文件
