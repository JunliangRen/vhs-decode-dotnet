...
.NET 11.0.100-preview.7.26381.103。输入是同一份私有的真实 PAL 40 MSPS `.ldf`
（21,389,175,871 字节，约 631 秒），且不会公开其文件名。
...
Exact 继续使用原有的托管 float32 表达式与运算顺序。该池最多保留 12 个 native
context，并随 RF pipeline 一起释放。专用 xUnit v3 集成测试覆盖 pipeline 路由、
context 创建、保留上限与释放。

在固定私有 PAL VHS 200 帧 trace 中，原先归属于 RF envelope 滤波的约 1.50 秒托管
SOS CPU 已消失，剩余约 0.44 秒托管 SOS 来自色度处理。两组正反顺序的 1000 帧
IPP-fast 配对在全部已采集产物和日志表面上保持一致。v0.4.0 profile 的墙钟为
46.282/46.331 秒，属于中性；平均 CPU 从 199.695 降到 198.227 秒（减少 0.74%）。
`current` 从 34.463 降到 34.224 秒（快 0.69%），CPU 从 216.430 降到
210.336 秒（减少 2.82%）。

另一次 1000 帧 runtime-counter 运行采集了 34 个工作集样本。首段/末段三分之一
中位数为 386.8/387.6 MiB，峰值 390.8 MiB，总分配 677.1 MiB，GC 暂停合计
35 ms。12 次确定性门禁覆盖 Exact/IPP-fast、v0.4.0/`current` 以及
`--threads 0`、默认 5、`--threads 20`；每个 profile 内的亮度、色度、原始 JSON、
stdout、归一化 stderr/日志和有序 `fileLoc` 全部一致。

### 无堆分配的内部 VHS burst-probe 结果

Phase 22 候选保留公开 `ChromaBurstDemodulationResult` 原有的 sealed record class。
审查否决了直接公开 class-to-struct 的原型，因为它会改变 CLR 签名、二进制绑定、
`null`/`default`、对象身份、装箱和泛型行为。最终路径为 11 个标量字段使用独立的内部
readonly record struct，仅在真正的公开 API 边界转换回公开 class。聚焦 xUnit 测试锁定
两种类型类别、正负零、NaN payload、无穷值、init-only 字段、精确转换和非破坏性的
`with` 行为。

七组反向聚焦配对保持精确 checksum，同时把串行分配量中位数从 114,244,800 降到
67,256,448 bytes（41.13%），四 worker 从 183,595,176 降到 137,502,920 bytes
（25.11%）。匹配的 200 帧 Exact `current --threads 20` GC trace 把总分配 tick 从
2,099 降到 1,998（4.81%），采样到的 `ChromaBurstDemodulationResult` tick 从 108
降为零；该 trace 的亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序
`fileLoc` 也全部一致。

20 组反向 200 帧配对、8 组正反顺序 1000 帧配对和下方 60 次矩阵的所有已采集表面
均完全一致。长跑配对墙钟变化范围为快 0.37% 到慢 1.01%，因此保留它作为分配和
GC 压力优化，不宣称端到端吞吐或峰值内存提升。

### 有界 libsndfile PCM16 重叠回退环

并行 VHS block 批次会读取互相重叠的 32,768-sample RF 窗口。直接 libsndfile 路径
此前会为每个可复用 block 向后 seek，并再次解码重叠部分。Phase 24 为每个
libsndfile loader 加入一块懒分配的 1,048,576-frame PCM16 环形回退窗口，固定为
2 MiB。`ReadReusable` 复制缓存前缀，仅向 libsndfile 请求新尾部。普通 `Read`
保留原 seek 行为；映射重启、真实原生 seek、fallback 和 dispose 都会清空该环。
PCM16 转换仍使用同一 short-to-double 表达式和 AVX2 转换路径。

Exact 与 IPP-fast 共六组交错 200 帧配对在亮度、色度、原始 JSON、stdout、归一化
stderr/日志和全部有序 `fileLoc` 上一致。Exact 墙钟中位数从 9.231 变为 9.223 秒，
CPU 中位数从 88.141 降到 84.906 秒；IPP-fast 墙钟中位数从 7.749 变为 7.757 秒，
CPU 中位数从 45.953 降到 45.406 秒。短程墙钟变化只作为吞吐中性的筛选证据。

四组相反顺序的 1000 帧配对继续保持全部表面严格一致。Exact 基线/候选平均墙钟为
35.383/35.027 秒，降低 1.00%；平均 CPU 时间为 293.797/290.023 秒，降低 1.28%。
IPP-fast 平均墙钟为 33.112/32.830 秒，降低 0.85%；平均 CPU 时间为
197.367/196.844 秒，降低 0.27%。候选峰值工作集保持在 353.0-357.5 MiB，峰值
private memory 保持在 366.9-371.5 MiB。回退环尺寸固定，fallback/dispose 时释放，
不会随解码长度增长。

另一个 12 次基线/候选门禁覆盖 Exact v0.4.0 和 `current` 的 `--threads 0`、省略参数的
默认五 worker 以及 `--threads 20`。全部采集表面与 main 一致，每个 profile 在各线程数
...
`--threads 20`，全部表面一致且每个 profile 保持确定。零警告 Release build 和 1443 项
标准 xUnit v3 套件门禁通过：1439 项通过、0 项失败，4 项仅 IPP 测试因该测试输出未携带
原生运行库而跳过。

### 并行准备托管 real-FFT inverse

最新 Exact VHS `current` 路径会按请求的 workers 并行准备大型托管 real-FFT inverse
中彼此独立的共轭配对。每个配对仍使用与串行循环相同的 float32 输入、twiddle 查询、
算术表达式、写入位置和中心 bin 覆盖。FFT、归一化、packet 调度和有界 worker 私有
workspace 均不变；小型变换和单 worker 调用仍走串行路径。

三组交错的 1000 帧 Exact `current --threads 20` release 二进制配对在退出状态、
亮度、色度、原始 JSON、stdout、归一化 stderr/日志和每个有序 `fileLoc` 上均一致。
三组候选分别快 6.25%、5.29% 和 1.04%。基线/候选中位数为 35.822/33.715 秒，
墙钟降低 5.88%，吞吐提升 1.063x。CPU 时间中位数从 298.891 降至 284.063 秒
（降低 4.96%），有效核心占用从 8.34 增至 8.43。峰值工作集中位数从
359.28 降至 348.45 MiB（降低 3.01%），峰值 private bytes 中位数从
373.59 降至 359.74 MiB（降低 3.71%）。

含启动开销较高的 160 帧 apphost 矩阵被保留为反例，而不是包装成普遍提速。在 15 组
同一时刻的基线/候选配对中，默认、1、5、10 workers 的候选中位数分别快 0.08%、
0.44%、1.85%、2.59%，20 workers 则慢 4.14%；七类采集面全部保持严格一致。
另一个单 worker 三组配对核对中，旧 `bdccd58` 二进制为 41.77 秒，当前 main 为
41.04 秒。因此相较上一次跨日期快照下降的倍数来自主机状态漂移，而不是 Phase 24
或 Phase 25 版本回退。另一个 12 次门禁覆盖 Exact v0.4.0/current 的 `--threads 0`、
省略参数的默认五 worker 和 `--threads 20`，全部表面与 main 一致且每个 profile 保持确定。

新增的三用例 xUnit v3 理论测试使用合法的 real-FFT 长度 32256、32768、33600，
分别覆盖 8192 个 pair 并行门槛的下方、门槛点和上方；20-worker workspace inverse
在三种长度下都与串行结果逐字节一致。完整本地套件发现 1446 项测试：1442 项通过、
0 项失败，4 项仅 IPP 测试因缺少原生运行库而跳过。

### 有界的跨场 VHS wavefront

2.1.0 版在生产 VHS 序列解码中加入容量为 2 的 wavefront。同步与有序状态规划仍在
序列线程执行；亮度渲染、已准备色度的完成阶段和磁带 dropout 映射可以并行，但只有
仍引用当前 RF span 的任务全部结束后才会开始下一场 RF 读取。输出、JSON、`fileLoc`、
恢复和诊断始终按输入顺序提交；逐场诊断捕获也会在下一次有序提交前恢复。

大型租赁 RF span 会在 lookahead 前归还，跨越场边界的只有独立拥有的色度尾部和小型
池化场输出。色度、渲染和输出池的容量均为 2。500 帧内存配对中，基线/候选峰值工作集
为 354.4/473.2 MiB，private bytes 为 365.7/525.0 MiB；1000 帧时分别为
353.6/468.9 MiB 和 367.2/502.8 MiB，证明这是固定窗口而非逐场增长。首版原型曾同时
保留两个完整 RF span，峰值接近 884 MiB；该版本在发布前已被淘汰。

Exact `current` 被明确排除在此 wavefront 外：门控后的三对交错 500 帧运行中位数为
19.74/19.86 秒，没有有效收益。保留路径则有稳定提升：Exact v0.4.0 从 33.55 降至
32.31 秒（墙钟缩短 3.7%）；IPP-fast `current` 从 19.77 降至 17.26 秒（缩短 12.7%），
后者有效核心数从 5.66 升到 6.58，而 CPU 时间仅增加约 1.5%。

最终 1000 帧 `--threads 20` release 二进制门禁在四种 Exact/IPP-fast 与
v0.4.0/`current` 组合中均匹配亮度、色度、原始 JSON、有序 `fileLoc`、stdout、归一化
stderr 和归一化日志。Exact v0.4.0 从 46.047 降至 42.575 秒，IPP-fast v0.4.0 从
44.980 降至 42.084 秒，IPP-fast `current` 从 31.009 降至 25.260 秒。最后一条路径的
CPU 时间从 188.20 变为 197.34 秒，有效核心数从 6.07 升到 7.81。另有 24 次门禁覆盖
...
下一轮 Exact-current 优化在 `TbcFieldDecodePipeline` 中保留调用方拥有的分类与
精修脉冲列表。公共 `SyncAnalyzer` 方法仍返回独立拥有的结果；只有内部串行场路径
会清空并复用这两个列表。脉冲顺序和值、rescue 修改、VBlank 状态、诊断以及全部
跨场状态转换均保持不变。
如果损坏场曾让 backing array 超过 65536 项，后续恢复到正常大小的场会在复用前
释放这份过大的历史保留。

在此之前测试过一个 worker-local PocketFFT scratch 原型；它保持逐位一致，但两组
顺序互换的 1000 帧配对中，基线/候选墙钟组中位数为 32.62/32.64 秒，CPU 时间为
297.09/299.14 秒，没有稳定的端到端价值，因此已完整回退。

最终保留的脉冲列表修改在同一份 160 帧 `gc-verbose` trace 中把采样托管分配从
680,536,552 降到 621,643,144 bytes（降低 8.65%），并消除了采样值为
65,614,400 bytes 的 `ClassifiedSyncPulse[]` 热点。六组短配对受调度噪声影响；两组
顺序互换的 1000 帧 `--threads 20` 配对则把墙钟中位数从 32.95 降到 32.11 秒
（降低 2.55%），CPU 时间从 298.40 降到 288.02 秒（降低 3.48%），有效核心数
从 9.06 变为 8.97。

两组顺序互换的内存配对中，候选峰值都更低。保守采用反序结果时，峰值工作集的
基线/候选为 390.8/360.5 MiB，private bytes 为 409.9/374.4 MiB。基线先跑的
一组出现 628.7 MiB 的基线 GC 峰值，因此只把它标为离群点，不据此宣称更大比例。
另有 24 次 release 二进制门禁覆盖四种 backend/profile 组合以及 `--threads 0`、
默认五 worker、20 workers，全部采集面均一致；刷新的 Preview 7 候选 60 次公开矩阵每格也只有
一套 hash，1,613 项标准 xUnit v3 测试套件通过。
...
2026-08-26 用基于 main commit `b893bf7` 并包含 current RF 色度 pass 优化的
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 52.811 s | 54.243 s | 11.598 s / 4.553x / 78.04% | 11.398 s / 4.759x / 78.99% | 10.230 s / 5.162x / 80.63% | 7.997 s / 6.783x / 85.26% |
| `--threads 1` | 57.067 s | 56.762 s | 32.969 s / 1.731x / 42.23% | 37.599 s / 1.510x / 33.76% | 22.629 s / 2.522x / 60.35% | 24.415 s / 2.325x / 56.99% |
| `--threads 5` | 52.920 s | 55.722 s | 11.773 s / 4.495x / 77.75% | 11.100 s / 5.020x / 80.08% | 10.101 s / 5.239x / 80.91% | 7.971 s / 6.991x / 85.70% |
| `--threads 10` | 52.965 s | 54.949 s | 9.040 s / 5.859x / 82.93% | 8.122 s / 6.765x / 85.22% | 8.391 s / 6.312x / 84.16% | 5.813 s / 9.453x / 89.42% |
| `--threads 20` | 53.555 s | 54.842 s | 7.310 s / 7.327x / 86.35% | 7.076 s / 7.751x / 87.10% | 6.919 s / 7.741x / 87.08% | 4.868 s / 11.265x / 91.12% |
<!-- LATEST_PERFORMANCE_END -->
<!-- LATEST_PERFORMANCE_PHASE159: kernel-ab-rounds=8 320-ab-pairs=3 1000-ab-pairs=3 public-cell-runs=60 tests=1615 -->
<!-- LATEST_PERFORMANCE_PHASE159_RUNS: dotnet-date=2026-08-26 dotnet-matrix-runs=60 dotnet-repeats=3 python-reference-date=2026-08-12 python-reference-runs=30 -->
<!-- LATEST_PERFORMANCE_PHASE159_EVIDENCE: kernel-old-new=28.025/16.166ms kernel-wall-gain=42.317% 1000-paired-wall-gain-median=0.5385% 1000-paired-cpu-gain-median=0.6860% compatibility-surfaces=8 -->
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 52.583-62.222 s | 53.893-58.195 s | 11.495-11.834 s | 10.811-11.412 s | 10.217-10.272 s | 7.899-8.057 s |
| `--threads 1` | 56.709-60.521 s | 56.335-58.991 s | 32.305-33.199 s | 37.596-37.621 s | 22.597-22.647 s | 24.266-24.460 s |
| `--threads 5` | 52.845-53.977 s | 53.696-58.437 s | 11.744-11.909 s | 11.090-11.226 s | 10.096-10.284 s | 7.944-8.053 s |
| `--threads 10` | 51.797-53.088 s | 52.649-56.775 s | 8.869-9.055 s | 7.717-8.239 s | 8.115-8.557 s | 5.769-5.889 s |
| `--threads 20` | 52.967-55.987 s | 53.005-55.618 s | 7.201-7.322 s | 6.711-7.348 s | 6.880-7.078 s | 4.828-4.970 s |
<!-- LATEST_PERFORMANCE_RANGES_END -->

保留的 30 次 Python 测量来自 2026-08-12 固定条件活动。全部二十个 .NET 单元格来自
同一个 .NET 11 Preview 7 候选二进制在 2026-08-26 完成的 60 次运行。每个三次运行
单元格内都只有一套亮度、色度、原始 JSON、stdout、归一化 stderr、归一化日志和
有序 `fileLoc` hash。Python v0.4.0 的 15 次运行产生了 15 套不同的亮度、色度、JSON 和
归一化日志 hash，因此严格 oracle 仍为 `g4315520 --threads 0`。

全部刷新的 .NET 单元格都使用基于 main commit `b893bf7` 并包含 current RF 色度 pass 优化的
Preview 7 自包含候选二进制；其 product version 为
`2.10.0+b893bf7d39edf574609a7abc40237a4db32eaf9f`，137,284,417 字节单文件
...
### Current RF 色度量化/移位融合

`current` 路径过去先把完整 double 缓冲区量化为 float32，再在 roll 时按源索引降序
累加 DC 均值，最后减去均值并进行第二次 float32 转换。保留候选把第一次量化融合进
roll 与累加。AVX 循环仍按原来的 `3, 2, 1, 0` lane 顺序相加，最终居中循环只向量化
彼此独立的减法和转换；不会使用 FMA，也不会重结合 reduction。禁用 AVX 或均值不是
有限数时仍走标量路径。

冻结旧实现的测试覆盖 13 种缓冲区长度、九种正/负/超范围 move、堆分配 wrapped 缓冲区、
不同 NaN payload、正负无穷、有限均值下的有符号零，以及 AVX-required 与禁用 AVX 两条
路径，所有输出 double 位均一致。八轮交错微基准中，
32,768 样本缓冲区运行 1,000 次从 28.025 ms 降至 16.166 ms（42.317%）。三组
正反顺序的 1,000 帧 Exact `current --threads 20` 配对在退出状态、亮度、色度、原始
JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 上全部一致；配对墙钟和 CPU 提升
中位数为 0.5385% 和 0.6860%。60 次公开矩阵也在五个 worker 设置的每个
profile/backend 中只产生一套 hash。内存波动处于正常运行噪声内，因此不声明降低。
Release 构建为 0 warning/0 error；标准 xUnit v3 共发现 1,615 项，其中 1,612 项通过，
3 项按预期因环境不可用跳过。

### Staged VHS payload 前缀 materialization

copy attribution trace 将 Exact 约 40.8% 的采样时间归到 VHS payload materializer。
在已有的 20-worker 分段 envelope 路径中，候选最初只复制由场请求与既有 line lookahead
估算出的 Video/Chroma 前缀；若实际有限行位置需要更多数据，再按完整源 block 扩展所有权。
数组、复制表达式、padding 与逐样本顺序不变；公开的完整 materialization、raw metric 和
DC-adjust 路径仍处理完整 span。
...
clean Exact `current --threads 20` trace 把 1.10% 的独占采样时间归到标量
`PocketFftComplex.Pair` helper，且直接父调用全部是 `Pass8FirstIndex`。相邻的向量重载和
原地 helper 已有 `AggressiveInlining`，本候选只为遗漏的标量 helper 补上同一提示。
加减法及其顺序、数据类型、FFT plan、缓冲区所有权、worker 策略和异常行为均不改变。

Exact `current --threads 20` 在 160、500、1,000 帧各完成三组交错配对；每次的退出码、
亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 均一致。1,000 帧
基线/候选各自的独立中位数为墙钟 30.213/29.576 秒、进程 CPU 290.719/279.297 秒；
三组逐对改善率的中位数分别为墙钟 1.35%、CPU 3.93%，基线/候选峰值工作集为
425.4/388.6 MiB。另行三组显式零线程配对的基线/候选独立墙钟中位数为
37.492/36.962 秒，逐对改善率中位数为 0.43%；高线程收益没有引入可测的低线程回退。

profile/thread 门禁覆盖 `v0.4.0` 和 `current` 的省略 `--threads`、显式零与 20 workers。
刷新的 60 次 Exact/IPP-fast 矩阵在每个单元格内部以及默认/1/5/10/20 workers 之间均
保持确定。Release solution 以 0 warning、0 error 构建；标准 xUnit v3 共发现 1,613 项，
其中 1,610 项通过，3 项按预期因环境不可用跳过。

### 线性 TBC level-adjust 单遍处理

本轮保留候选把线性 TBC level-adjust 数组的构建和指数平滑合并为一次按索引递增的
写入，不再先填满完整的原始因子数组再进行第二次扫描。中位数/MAD 异常值 clamp、
精确的 `Math.FusedMultiplyAdd` 表达式、逐样本求值顺序、double 转换点、行边界和
worker 所有权均不变；禁用 smoothing 的调用继续使用原来的 `Array.Fill` 路径。

递进门禁包括三组交错的 160 帧配对、两组 500 帧配对，以及两组正反顺序的 1,000 帧
Exact `current --threads 20` 配对。每次运行的退出状态、亮度、色度、原始 JSON、
stdout、归一化 stderr/日志和全部有序 `fileLoc` 都完全一致。在 1,000 帧门禁中，两组
均由候选获胜：墙钟中位数从 29.758 降至 28.799 秒（缩短 3.22%），进程 CPU 时间
中位数从 287.672 降至 281.703 秒（缩短 2.05%）。

一次仅候选的 2,000 帧运行用时 54.408 秒，平均使用 9.82 个有效核。峰值 private
memory 为 588.6 MiB，低于 1,000 帧候选门禁观察到的 637.7 MiB 最大值，采样时间线
还包含 4 次超过 1 MiB 的回落。这证明了本次测量窗口内的有界行为，但不声明普遍的
常驻内存下降。

显式零线程和省略参数的默认五 worker 运行均在自身内部保持确定，并在七项采集表面上
与 20-worker 候选输出一致。刷新的 60 次 Exact/IPP-fast、v0.4.0/`current`、
default/1/5/10/20 矩阵也在每个单元格内和跨 worker 设置保持一套 hash。TBC、转换和
current 色度聚焦套件在正常模式以及适用的禁用 AVX2/全部硬件 Intrinsic 模式下均通过。
标准 xUnit v3 发现 1,613 项测试：1,610 项通过，3 项依赖环境的测试按预期跳过。

### 复数解析信号逆 FFT 的缓冲区所有权复用

本轮保留的 Exact 候选去掉了复数解析信号逆 FFT 前的保留式复制。Hilbert 和 RF
滤波器完成原地处理后，已准备频谱不再需要保留，因此
`PocketFftComplex.InverseOwned` 直接把该数组和第二个 worker 私有数组作为交替 pass
存储。它返回最终 pass 所在的数组；VHS workspace 随后把该数组继续视为
`FullAnalytic`，另一个数组则成为可复用的 `DiffedAnalytic` scratch。整个过程不增加或
额外保留数组，PocketFFT factorization、蝶形、twiddle、归一化、AVX 适用条件、
浮点类型和求值顺序均不变。

递进门禁包括三组交错的 160 帧配对、两组 500 帧配对，以及两组正反顺序的
1,000 帧 Exact `current --threads 20` 配对。每组的退出状态、亮度、色度、原始 JSON、
stdout、归一化 stderr/日志和全部有序 `fileLoc` 都一致。1,000 帧两组均由候选获胜：
墙钟中位数从 28.804 降至 28.549 秒（缩短 0.89%）；进程 CPU 时间基本不变，为
283.367 对 283.320 秒（缩短 0.02%），符合“减少关键路径内存复制而不减少算术量”的预期。

最初一次基线先跑的 `--threads 0` 探针噪声较大，因此固定确认只追加两组正反顺序
配对后即停止。确认中位数为 34.305/34.269 秒，属于中性；七个表面全部一致，候选
峰值 private memory 略低。聚焦的存储与并行 RF 测试在正常模式、禁用 AVX 和禁用
全部硬件 Intrinsic 时均通过。

一次仅候选的 2,000 帧运行用时 54.184 秒，平均使用 9.80 个有效核。峰值 private
memory 为 643.3 MiB，时间线有 4 次超过 1 MiB 的回落，最后 6 秒稳定在
634.4-634.7 MiB。这只是有界测量窗口的观察，不宣称普遍降低内存。刷新的 60 次
Exact/IPP-fast、v0.4.0/`current`、default/1/5/10/20 矩阵在每个单元格和跨 worker
设置都保持一套 hash。标准 xUnit v3 共发现 1,613 项测试：1,610 项通过，3 项依赖
环境的测试按预期跳过。

### AVX 实数到复数 FFT 输入 staging

本轮保留候选把复数 PocketFFT 路径中的标量实数输入 staging 循环改为每次四个样本的
AVX 交错写入。load、unpack 和 128-bit lane permutation 仍写出完全相同的
`[real, +0.0]` 分量顺序。这里没有改变浮点运算、FMA、归约、FFT factor、蝶形、
twiddle、归一化或求值顺序；长度为二和尾部样本仍使用原标量表达式，非 AVX 机器
完整走标量路径。调用方输入输出重叠时，也仍先使用既有 plan scratch 再做 staging。

八轮 32,768 点完整 FFT 探针得到相同 SHA-256，耗时中位数从 219.493 降至
216.729 ms（缩短 1.26%）。真实 RF 门禁随后使用三组交错 160 帧配对、两组正反
顺序 500 帧配对，以及两组正反顺序 1,000 帧 Exact `current --threads 20` 配对；
每个长度上的七个兼容面都一致。两组 1,000 帧的基线/候选合并墙钟中位数为
34.711/33.171 秒（缩短 4.44%），CPU 中位数为 294.758/290.016 秒（缩短 1.61%）。
其中一组墙钟由基线快 1.27%，因此结论仅为正常调度波动下的总体小幅提升。

候选在显式零线程、省略参数的默认五 worker 和 20 workers 下，各自重复运行以及
...
本轮保留候选针对常见的四 section 托管 float32 SOS 路径：中心样本直接写入调用方
目标，只把奇延拓的两侧边缘保留为 scratch。系数转换、初始条件、正反向 section
顺序、逐样本表达式顺序、padding 和 IPP 路由均不变；其他 section 数继续使用原来的
扩展缓冲路径。

因果门禁使用同一提交、SDK、机器和 `FolderProfile` 构建的本机干净 main 与候选二进制。
三组交错的 1,000 帧 Exact `current --threads 20` 配对在亮度、色度、原始 JSON、
stdout、归一化 stderr/log 和全部有序 `fileLoc` 上一致。墙钟中位数从 29.63 降至
28.96 秒（缩短 2.3%），CPU 时间从 281.06 降至 276.69 秒（缩短 1.6%）。三组
候选的 CPU 时间都更低，墙钟则赢两组、输一组。直接热调用的托管分配低于 64 字节，
且没有队列或随输入长度保留的 workspace。

刷新的 default/1/5/10/20 Exact/IPP-fast 60-run 矩阵在每个单元格、每个记录表面都
只有一个 hash。聚焦的 46 项 DSP 测试在启用 Intrinsic 和禁用全部硬件 Intrinsic 时
均通过；标准 xUnit v3 套件为 1,607 项通过、3 项环境跳过。

### 重新平衡 current VHS 逆变换调度

新的 320 帧 Exact `current --threads 20` CPU trace 显示，活跃 RF block 工作约为
36.6 CPU 秒，inverse companion lane 约为 14.2 CPU 秒；原来的 12 个 outer worker
加 8 个专用 companion 会让 companion lane 偏空。本轮保留的策略在请求 20 workers
时改用 14 个 outer worker 和 6 个 companion。28-block lookahead 以及既有 cache、输出、
workspace 上限均不变；v0.4.0、少于 20 workers、GNRC 和 sharpness 保持原调度。

这只改变已经相互独立的 inverse 由哪个有界 worker 承担。FFT plan、buffer、运算、
异常优先级、跨场状态、输出、元数据和诊断均未改变。两组反转先后顺序的 1,000 帧
Exact `current` 配对在亮度、色度、原始 JSON、stdout、归一化 stderr/log 和全部有序
`fileLoc` 上一致。合并中位数从 32.962 降至 32.699 秒（墙钟缩短 0.80%），CPU 时间
从 305.711 降至 296.094 秒（缩短 3.15%）。由于 CPU 工作量下降快于墙钟时间，
有效核心数从 9.27 变为 9.06；峰值工作集保持有界，约为 395-397 MiB。

四组交错的 160 帧 IPP-fast `current` 配对也在同七项表面上一致，墙钟中位数从
8.752 降至 8.584 秒（缩短 1.92%）；CPU 时间基本不变，有效核心数从 5.85 提高到
5.95。独立的 v0.4.0 Exact 与 IPP-fast 配对均为中性。刷新的 60 次
default/1/5/10/20 矩阵中，每个单元格的每项表面都只有一个 hash。Release 测试在原生
Intrinsic 下为 1,607 通过、3 个环境跳过；禁用 AVX2 时为 1,607/3；禁用全部硬件
Intrinsic 时为 1,606/4。

### Exact 高 worker 同步预处理

保留的候选仅对请求至少 8 个 worker 的 Exact `current` 路径，把 VHS boxcar/同步预处理上限从
4 提高到 8。默认 5-worker 与全部 IPP-fast 路径继续使用原来的 4-worker 上限。DSP
表达式、运算顺序、场状态、有序提交、元数据和诊断均未改变。

独立同步检测器 A/B 的墙钟中位数从 651.775 降到 404.724 ms（1.610x），输出完全
一致。六组交错的 160 帧 Exact `current --threads 20` 配对把墙钟中位数缩短约 2.4%；
一组 1,000 帧配对从 33.760 降到 32.946 秒（墙钟缩短 2.41%；吞吐 1.0247x，即提升
2.47%），有效核心数从 8.62 增至 9.16。亮度、色度、原始 JSON、stdout、归一化
stderr/日志及有序 `fileLoc` 全部一致；
峰值工作集约增加 2.5 MiB，仍保持有界。最初让 IPP-fast 也使用 8-worker 的版本慢了
3.2%，因此已撤回，IPP-fast 继续保持 4-worker 上限。

最终 60 次矩阵覆盖 Exact/IPP-fast、v0.4.0/`current` 以及默认、1、5、10、20 workers，
每格三次运行；每个单元格的七项采集表面都只产生一套 hash。Release 构建为零警告、
零错误；暂存固定 IPP runtime 后，标准 xUnit v3 共发现 1,609 项测试，其中 1,606 项
通过，3 项因 CUDA/AMF 环境不可用而按预期跳过。

### 托管 AVX Super-Gaussian 频谱掩码

托管 Exact 色度最终滤波器现在使用 AVX，每次把 Super-Gaussian 频谱掩码应用到四个
`Complex32` 值。每个 float 分量先扩展为 double，并逐 lane 保留原标量乘法、减法/加法
以及最终 double 到 float 的转换点。循环不使用 FMA 或归约。任何向量写回前都会检查
两个 double 结果；只要一个 lane 为 NaN 或无穷，该向量及剩余尾部就交给原来的标量
JIT 形态。未对齐 Span，以及 1、2、3 个值的尾部也保持相同标量行为。IPP 掩码路径未改动。

8 对交错的进程级内核测试各自对 178,201 点掩码执行 2,000 次，全部 16 次运行产生同一个
SHA-256。标量墙钟中位数为 1,210.850 ms，AVX 为 167.083 ms，即缩短 86.20%、吞吐
提升 7.247x；CPU 时间中位数降低 86.18%。release 反汇编只出现 `vmulpd`、`vsubpd`
和 `vaddpd`，没有融合乘加。聚焦测试覆盖 539 组有限值、全部向量尾部长度、未对齐切片
和前后哨兵，以及完整的 12 x 12 x 12 异常值交叉组合。默认 JIT、
`TieredCompilation=0`、强制禁用 AVX 和禁用全部硬件 Intrinsic 的证据，均与各自基线的
标量结果一致。

6 对交错的 1,000 帧 Exact `current --threads 20` release 二进制运行，在退出状态、亮度、
色度、原始 JSON、stdout、归一化 stderr/日志和全部有序 `fileLoc` 上一致。候选赢得 5 对，
另 1 对仅慢 0.03%。独立中位数的墙钟从 35.879 秒降到 35.608 秒（0.76%，1.008x），
CPU 时间从 298.148 秒降到 295.141 秒（1.01%），有效核心数从 8.18 升到 8.38
（2.43%）。峰值工作集中位数从 352.5 降到 345.9 MiB，private bytes 从 364.8 降到
357.9 MiB。

最终 release 二进制通过 12 个 Exact、12 个 IPP-fast 配置/线程门禁，以及原生、禁用 AVX、
禁用全部 Intrinsic 的 Exact 门禁；全部七项产物/日志表面与有序 `fileLoc` 均一致。新的
60 次首页矩阵为每个 backend/profile/thread 单元格执行三次完整运行，每个单元格只产生
一套 hash。先前一批测量因出现无关高 CPU 进程而被丢弃；最终接受的批次在每次解码前
都要求外部 CPU 采样足够低。完整 xUnit v3 套件的 1,448 项测试全部通过。

### 实数逆 FFT 最终 radix-4 向量化与解析缓冲区去复制

解析信号的差分修复不再先把完整缓冲区复制到第二个数组再覆盖。改为降序原地处理后，
每次减法读取的源值不变，分配、padding 与后续所有权语义也保持不变。4 对交错的
1,000 帧 Exact `current --threads 20` 运行在全部兼容性表面上一致；墙钟中位数从
39.174 秒变为 39.152 秒，基本中性（0.06%），CPU 时间中位数从 315.130 秒降到
310.140 秒（1.58%）。

32K 实数逆 FFT 中常见的最终 `ido == 1` 阶段现在用 AVX 同时处理四个彼此独立的
radix-4 蝶形。加载值先转置为原来的蝶形布局，每个 lane 保留标量版本的操作数及乘法
顺序；不使用 FMA 或水平归约，标量尾部和禁用 AVX 时的回退保持不变。专用 xUnit v3
oracle 用带符号零、subnormal、最大有限数、无穷以及 signaling/quiet NaN 对完整逆变换
结果与标量实现逐 bit 比较。

8 对交错的进程级内核测试每次执行 4,000 个预热后的 32K inverse，全部产生同一个
SHA-256。墙钟中位数从 697.821 ms 降到 676.900 ms（3.00%）。之后 4 对交错的
1,000 帧 Exact `current --threads 20` release 二进制运行在亮度、色度、原始 JSON、
stdout、归一化 stderr/日志以及全部有序 `fileLoc` 上一致，候选胜出 4/4。墙钟中位数
从 32.971 秒降到 32.323 秒（1.96%，1.020x），CPU 时间从 278.195 秒降到
267.500 秒（3.84%）。

独立的 1,000 帧采样运行峰值工作集为 394.2 MiB，private bytes 为 404.5 MiB；首三分之一
和末三分之一中位数不升反降，没有观察到缓冲区滞留或 OOM 趋势。Exact/IPP-fast、
v0.4.0/`current`、`--threads 0`/默认 5/`--threads 20` 随后通过 24 次 release 二进制
兼容门禁，覆盖原生及标量 FFT 路径。刷新的 60 次首页矩阵在五种 worker 模式下每个
profile 都只保留一套 hash。完整 xUnit v3 套件的 1,448 项测试全部通过。

### AVX2 CTI 倒数查表 gather

`current` CTI 距离阶段继续使用固定的 2048 项倒数尾数表和相同的 float32 位构造。
在支持 AVX2 的主机上，八个彼此独立的 bucket 索引现在通过一次 `GatherVector256`
读取；符号位、指数位、零/subnormal 到无穷、高指数到带符号零，以及 NaN payload
quiet 化都用逐 lane 整数位运算重建。没有 FMA、归约、重结合、共享 scratch 或新的
逐样本分配；原标量查询继续作为回退。

聚焦 xUnit v3 测试覆盖全部 2048 个表 bucket、正负符号、零、subnormal、指数边界、
无穷、signaling/quiet NaN payload 和标量尾部；原生 AVX2 与强制禁用 AVX2 两种运行
均通过。八组交错、每次 40 轮的内核配对在全部 16 次运行中只有一套 SHA-256。
墙钟中位数从 1763.352 降至 912.073 ms（减少 48.28%，吞吐 1.933x），CPU 时间
中位数从 1710.938 降至 906.250 ms（减少 47.03%）。

四组交错 1000 帧 Exact `current --threads 20` release 二进制配对在退出状态、2000 个
fields、亮度、色度、原始 JSON、stdout、归一化 stderr/日志和全部有序 `fileLoc` 上
一致，候选胜出 4/4。墙钟中位数从 34.068 降至 33.462 秒（减少 1.78%，吞吐
1.018x），CPU 时间从 291.781 降至 286.734 秒（减少 1.73%），有效核心从 8.56
增至 8.57。峰值工作集中位数从 345.9 增至 347.4 MiB，private bytes 从 358.1
增至 359.1 MiB；四次独立运行中观测到的峰值保持有界。

最终 release 二进制矩阵覆盖 Exact/IPP-fast、v0.4.0/`current`、`--threads 0`、默认
5 和 `--threads 20`，并分别验证原生及强制禁用 AVX2 的候选路径。全部 36 次运行
在九项兼容面及跨线程确定性上完全一致。刷新的 30 次 `current` 首页矩阵在五种
worker 模式下每个 profile 都只有一套 hash。加载原生 IPP runtime 后，完整 xUnit v3
套件的 1447 项测试全部通过。

### 有界 current VHS companion inverse 调度

高 worker VHS `current` 原先会从每个 RF block worker 通过 `Task.Run` 启动 analytic
companion inverse，随后同步等待。采样显示存在嵌套 ThreadPool 调度和被阻塞的外层
...
减少 1.61%。另一次 1000 帧配对在同样表面上全部一致，墙钟从 34.732 降至
34.000 秒（减少 2.11%，吞吐 1.022x）；CPU 时间从 284.766 降至 279.500 秒，
有效核心从 8.199 增至 8.221。候选前半/后半工作集峰值为 352.2/351.4 MiB，
有界调度器没有逐步增长或 OOM。
...
16 次加法通过标量 SSE2 intrinsic 直接展开，每个新 tap 固定为左操作数，与基线的
NaN payload 顺序一致；同时省去对随后会被完全覆盖的栈缓冲区做冗余清零。标量和
边界路径完全不变。

八组交错单线程内核配对保持相同 SHA-256，批次中位数从 259.624 降至 233.064 ms
（减少 10.23%，吞吐 1.114x），候选胜出 8/8。产品决策仍以完整解码器为准：三组交错执行的 1000 帧
Exact `current --threads 20` 配对在退出状态、field 数、亮度、色度、原始 JSON、
stdout、归一化 stderr/日志和有序 `fileLoc` 上全部一致，候选胜出 3/3。平均墙钟从
35.242 降至 35.179 秒（减少 0.18%，吞吐 1.002x）；平均 CPU 时间从 281.84 增至
286.22 秒（增加 1.55%），平均活跃核心从 8.00 增至 8.14。

24 次确定性门禁覆盖 Exact/IPP-fast、v0.4.0/`current` 以及 `--threads 0`、默认 5、
`--threads 20`；每个产品表面都与基线一致，各线程模式之间也一致。默认、禁 FMA、
禁 AVX 和完全标量的真实 RF 运行同样逐项一致。不同正负 NaN payload、无穷和
有符号零也与保存的 main 二进制逐 bit 一致。34 项 TBC 聚焦测试分别在正常
intrinsic、禁用 AVX 和禁用全部硬件 intrinsic 下通过。完整 xUnit v3 套件通过
1429 项，另有四项本地 IPP runtime 测试按预期跳过。

另一次 2000 帧计数器门禁精确完成全部 4000 个 fields，墙钟从 68.724 降至
68.473 秒（减少 0.37%，吞吐 1.004x）。候选工作集中位数 353.1 MiB、最大
359.6 MiB，首段/末段三分之一中位数为 352.0/357.2 MiB。九个启动后的
200 帧区间保持在 6.446-6.699 秒，因此吞吐没有逐段衰减。总分配 1302.4 MiB，
GC 暂停合计 73 ms。

### 托管 AVX CTI 行快照

CTI 在四个 pass 的每一次开始前，都会把 float64 色度行刷新为 float32 快照。这个隔离
改动复用已经验证的托管 AVX 转换 helper，每轮同时把八个 double 转成八个 float；原有
cast、padding、标量尾部、目标 buffer、pass 边界和不支持 AVX 的行为保持不变，也没有
改变 CTI 运算、状态、调度或分配。

18 项 CTI 聚焦测试在正常 intrinsic、禁用 AVX 和禁用全部硬件 intrinsic 下全部通过。
转换 special-value 测试也在原生和禁用 AVX 路径通过，逐 bit 比较正负零、subnormal、
无穷、NaN、未对齐目标和标量尾部。完整 xUnit v3 套件通过 1,428 项，另有四项本地
IPP runtime 测试按预期跳过。

八组交错执行的长 CTI 内核配对保持一套输出 hash 和相同分配量，候选 8/8 胜出：墙钟
中位数从 2,522.737 降到 2,396.830 ms（减少 4.99%，吞吐 1.053x），CPU 中位数从
2,515.625 降到 2,390.625 ms（减少 4.97%）。

三组逆序执行的 1000 帧 Exact `current --threads 20` 配对在退出状态、field 数、亮度、
色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 上全部一致，候选 3/3
胜出；平均墙钟从 36.603 降到 35.631 秒（减少 2.66%，吞吐 1.027x），进程 CPU 从
297.333 降到 283.583 秒（减少 4.62%）。另 24 次运行覆盖 Exact/IPP-fast、
v0.4.0/`current`、`--threads 0`/默认 5/`--threads 20`；每组基线与候选以及每个
profile 的线程模式都在全部兼容表面上一致。

另一次 2000 帧候选运行在 69.119 秒内完成全部 4000 个 fields。峰值工作集为
355.75 MiB，最终为 353.52 MiB，首段/末段四分之一均值为 341.80/350.72 MiB，
未出现持续增长的峰值或 OOM。这是局部计算量下降，不作为多核利用率提升的结论。

### 之前的托管 AVX double-complex 逆 FFT 归一化

这个保留的历史候选基于已合并 main `ea1bb8e`。

逆变换会把每个实部和虚部乘以同一个双精度归一化因子。候选使用一个
`Vector256<double>` 同时处理两个相互独立的复数值，并保持标量操作数顺序。它不使用
FMA、归约、重结合、共享状态或新分配；原来的标量循环仍负责尾部与不支持 AVX 的路径。
JIT 汇编包含一条 `vmulpd`，没有 FMA 指令。

专用逐位等价测试覆盖长度 2、4、8、32 和 512 的变换，并包含正负零、subnormal、最小
normal、正负一、最大有限值、无穷以及不同 NaN payload。五项存储测试在正常 intrinsic、
禁用 AVX 和禁用全部硬件 intrinsic 下均通过。完整 xUnit v3 套件通过 1,426 项，另有四项
因 IPP runtime 不可用而按预期跳过。

八组顺序相反的长度 32,768 逆变换内核配对，每次测量执行 3,000 次变换。墙钟中位数从
986.10 降到 727.61 ms（减少 26.2%），CPU 中位数从 1,000 降到 750 ms（减少 25.0%）。
输出 hash 完全一致，两个版本都分配 616 bytes。

三组顺序相反的 1000 帧 Exact `current --threads 20` 配对在九个兼容表面上全部一致：
亮度、色度、JSON、stdout、归一化 stderr 与日志、有序 `fileLoc`、帧数和元数据。平均墙钟
从 39.867 降到 39.515 秒（减少 0.88%），CPU 从 315.260 降到 305.969 秒（减少 2.95%）。
三组墙钟方向都对候选有利，分别为 -2.10%、-0.20% 和 -0.35%。平均活跃核心数从 7.91
变为 7.74；这是局部计算量下降，不作为多核扩展能力提升的结论。峰值工作集保持有界，
基线和候选观察到的最大值分别为 650.6 MiB 与 396.2 MiB，未发生 OOM。

### 托管 AVX/AVX2 radix-11 PocketFFT butterfly

托管 float32 PocketFFT 的 radix-11 pass 现在用 AVX 同时计算四个连续频率索引；当
radix 11 是最终 stage 时，则使用 AVX2 gather 同时读取四个独立 packet，并连续写出结果。
每个 lane 都保持原标量 pair、系数乘法、加减、复数 twiddle 和转换顺序。超过保守溢出
界限的有限输入、非有限输入、标量尾部以及不支持所需 ISA 的主机继续执行原标量路径。
没有引入 FMA、归约、重结合、共享状态或新的样本级分配；unsafe 仅用于固定数组后的
packet gather/store。

61 项 mixed-radix 兼容测试在正常 intrinsic、禁用 AVX2、禁用 AVX 和禁用全部硬件
intrinsic 下均通过。专用直接 plan 测试在 forward/backward 两个方向覆盖长度 55、121
和 363，并包含正负零、subnormal、最小 normal、最大有限值、无穷和不同 NaN payload。
两次独立的长度 363、每次 10 组配对内核测试保持输出 bit 完全一致，中位数的中位数从
103.415 降到 34.925 ms（减少 66.23%）。这是隔离内核证据，不是端到端提升结论。

与 main `dbc617e` 顺序相反的三组 1000 帧 Exact `current --threads 20` 配对在九个兼容
表面上全部一致，候选 3/3 胜出。平均墙钟从 37.328 降到 36.734 秒（减少 1.59%，
吞吐 1.016x），进程 CPU 从 295.823 降到 293.208 秒（减少 0.88%），平均活跃核心数
从 7.925 变为 7.982。另一次候选 2000 帧运行在 70.139 秒内完成全部 4000 个 field，
并得到 4000 个唯一有序 `fileLoc`。峰值工作集为 647.43 MiB，结束时为 604.34 MiB，
三个三分区间的工作集均值为 590.37/598.75/601.05 MiB，跳过前 20% 后的线性斜率为
-24.81 MiB/min；未观察到持续增长或 OOM。

### 之前的托管 AVX radix-3 PocketFFT butterfly
...
和 -2.46%；基线和候选的有界最大工作集分别为 443.4 MiB 与 355.6 MiB。三组
...
尾部和不支持 AVX 的主机继续执行未改动的标量循环。向量路径不包含 FMA、归约、重结合、
共享 scratch 状态、分配或跨 transform 状态。

58 项 mixed-radix 兼容测试在正常 intrinsic、禁用 AVX 和禁用全部硬件 intrinsic 下均通过。
新增的固定标量 oracle 测试使用长度 3,840 的 transform，覆盖正负零、正负最小 subnormal、
正负最小 normal、正负三分之一和正负一。forward 与 backward SHA-256 在三种 intrinsic
模式下完全一致。

两组顺序相反的 1000 帧 Exact `current --threads 20` 配对合并后，平均墙钟从 36.800
降到 36.149 秒（低 1.77%），CPU 从 308.508 降到 298.828 秒（低 3.14%）。两组交错的
160 帧 Exact 配对也由候选获胜；两组 IPP-fast 配对的墙钟保持中性。顺序相反的
`--threads 0` 和 `--threads 1` 检查输出完全一致，合并后的单 worker CPU 基本持平，
因此不宣称串行加速。

刷新矩阵的 30 次运行在每个后端的默认、1、5、10、20 workers 之间，亮度、色度、原始
JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 全部一致。候选两次 1000 帧峰值为
352.1 和 450.5 MiB，基线为 370.6 和 370.2 MiB；矩阵最大值是单 worker 下两版本共同的
817.0 MiB，未见渐进增长或 OOM。

### AVX current 色度 ACC 分段缩放

`current` VHS 自动色度增益路径现在用托管 AVX 同时缩放四个相互独立的样本。gain 递推
继续以标量方式按原顺序执行，再构造四值向量。所有有限值组保持原有 double 到 float32、
double 乘法以及结果到 float32 的转换点；非有限值组和不支持 AVX 的主机执行原标量表达式。
没有 FMA、归约、重结合、状态边界移动、新数组或额外的池化生命周期。

15 项聚焦 xUnit v3 测试在正常 intrinsic、禁用 AVX 和禁用全部硬件 intrinsic 下均通过。
逐位测试覆盖向量尾部、正负零、subnormal、最大有限值、无穷和不同 NaN payload。JIT 检查
确认 gain 加法仍为标量，只使用 AVX 转换与乘法指令，且没有 FMA 指令。

两次独立会话中的五组交错 1000 帧 Exact `current --threads 20` 配对有四组由候选获胜。
两组会话把墙钟中位数从 34.782 降到 34.446 秒（低 0.97%），CPU 从 295.266 降到
291.602 秒（低 1.24%）。三组会话把墙钟从 35.255 降到 35.052 秒（低 0.57%）；CPU
中位数受噪声影响高 2.20%，因此不宣称 Exact CPU 效率提升。两组 IPP-fast 配对把墙钟
从 31.764 降到 31.247 秒（低 1.63%），CPU 从 202.539 降到 195.578 秒（低 3.44%）。

所有配对的退出状态、亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc`
均一致。额外门禁在 Exact 和 IPP-fast 下覆盖 `--threads 1`、显式零、默认 5 和
`--threads 20`。候选和基线峰值内存都保持在已有约 715 MiB 的观察进程窗口内，未见
渐进增长或 OOM。
...
托管 double 精度 complex FFT plan 现在根据 factor pass 的奇偶，把调用者输出或 worker-local
scratch 选为初始 source。每一遍仍在相同两个 span 之间交替，但最后一遍必定落到调用者输出，
从而移除原来的最终整数组复制和第二个 worker-local `Value[]` buffer。因子分解、twiddle、
radix 顺序、数据类型、归一化和每个浮点表达式均未改变，也没有引入 FMA 或重结合。
complex 输入/输出重叠继续采用等价于 memmove 的 span copy；real 输入存储与 complex 输出
...
一致。之后带重叠保护的最终 build 测得 1498.46、1516.92、1560.97 ms，覆盖此前候选
中位数，确认正常非重叠热路径保留了收益。

最终候选的 4 组 160 帧 Exact `current` 5-workers 配对在退出状态、field 数、亮度、色度、
原始 JSON、stdout、归一化 stderr/log 和有序 `fileLoc` 上全部一致，候选赢 3 组。墙钟
中位数从 20.72 降至 20.36 秒（低 1.73%），进程 CPU 时间从 84.20 降至 83.51 秒
（低 0.83%），峰值工作集中位数从 370.5 降至 364.2 MiB。该同一时刻 A/B 是 5-workers
路径的因果门禁；刷新后的公开表中倍率较低来自跨批分母/计时变化，不是候选回退。

正常生产路径的两组正反顺序 1000 帧 Exact `current --threads 20` 配对各产生 2000 个
有序 field。墙钟中位数从 41.10 降至 40.12 秒（低 2.38%），CPU 时间从 336.77 降至
328.91 秒（低 2.33%），峰值工作集从约 439.8 降至 416.7 MiB；九类兼容表面全部一致。
公开 90-run 矩阵中，每种 .NET profile/mode 和 Python PR341 profile/mode 也都只有一套
hash。Python v0.4.0 的 15 次运行每次都产生不同的亮度、色度、JSON 和归一化日志 hash，
因此严格 oracle 仍是 `g4315520 --threads 0`。

### 无边界检查的托管 float32 SOS 状态访问

托管 Exact float32 SOS 通用内核保留已经验证的每节两个 float 状态布局，但通过受 GC 跟踪的
span 引用访问递归状态。JIT 反汇编确认，正向和反向内层循环中的两个状态边界分支均已消失，
标量乘法、加法和减法顺序保持不变；没有引入 FMA、SIMD 重结合、转换、padding、生命周期或
所有权变化。

6 组交错的 160 帧 Exact `current --threads 20` 配对在九类产物、元数据、控制台和归一化日志
表面上全部一致，候选赢得 5 组。墙钟中位数从 11.499 降至 11.373 秒，低 1.10%；吞吐提升
1.12%，CPU 时间从 92.133 降至 90.695 秒，低 1.56%。4 组默认 5 workers 配对从 21.651
降至 21.555 秒，低 0.44%，候选赢得 3 组；0.74% 的 CPU 增幅和工作集变化均在短跑噪声内。
这些 A/B 脚本未传入 `--start`，因此使用 CLI 默认值 `--start 0`；其 160 帧时间不能与从
第 100 帧开始的矩阵直接比较。

1000 帧 Exact `current --threads 20` 配对从 42.463 降至 42.221 秒，墙钟减少 0.57%；CPU
时间从 351.297 降至 347.672 秒，低 1.03%，峰值工作集从 439.2 降至 437.7 MiB。这仅证明
本门禁内存有界，不构成普遍内存降低声明。6 次基线/候选线程门禁覆盖 `--threads 0`、默认 5
和 `--threads 32`，九类表面全部一致。

独立的受检查索引 float32 参考测试使用固定位模式，覆盖 5、8、10、31、32、33 和 64 个
sections；其聚合 SHA-256 为
`61A8966C55B1331509E0A60D6B731FA125F9406185C2C4AAF4E4CBC42F16C80D`，并在正常模式及禁用
全部 .NET hardware intrinsics 时通过。

### 无整段反转的托管 float32 SOS 第二遍

托管 Exact float32 前后向 SOS 路径原先依次执行正向滤波、整段反转、再次正向滤波，
最后再反转回来。现在第二遍从原 span 的最后一个样本走到第一个样本，并把结果写回同一
索引；它看到的样本顺序和端点初始条件完全相同，同时移除了两次完整缓冲区反转。1、2、
4 节及通用节数内核仍保持相同的逐样本节顺序、表达式、转换点和递归状态更新；没有引入
FMA、SIMD 重结合或精度变化。
该较早候选基于已合并的 main `73dd014`；被测源码 blob 为
`c3aac76922ad525884cf26779b2035db76248547`，基线 blob 为
`766e0926fec7c07bb2bc0a20c0c39824b8f05996`。

21 组正反顺序的 32K 微基准配对覆盖 2、4、5 个 SOS sections。基线/候选中位数分别为
154.540/151.793 微秒（低 1.78%）、195.178/190.025 微秒（低 2.64%）和
278.516/275.540 微秒（低 1.07%），所有输出 SHA-256 均一致。40 项 DSP 聚焦测试在正常
模式及禁用全部 .NET hardware intrinsics 时各通过一次，共 80 次标准 xUnit v3 执行。

8 组交错的 160 帧 Exact `current` 配对中，候选赢 6 组；配对中位墙钟时间下降 2.43%，
吞吐提升 2.49%，CPU 时间下降 2.69%。另一次 Exact v0.4.0 的 160 帧配对也完全匹配。
每组的亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 都一致。

1000 帧 Exact `current` 配对从 39.242 降到 37.908 秒，墙钟减少 3.40%；CPU 时间从
330.656 降到 310.141 秒，减少 6.20%。基线首段/末段三分之一工作集中位数为
385.7/387.7 MiB，峰值 391.3 MiB；候选为 380.9/384.2 MiB，峰值 387.8 MiB。这证明
本门禁内存有界，不构成普遍内存降低声明。6 次线程门禁覆盖 `--threads 0`、默认 5、
`1`、`5`、`10` 和 `20`，所有模式的七类兼容表面完全一致。
...
Exact VHS 解析路径原先会完整遍历 complex 频谱三次，依次应用 RF video filter、MTF filter
和 real Hilbert 缩放。现在按同一顺序在一次遍历中完成三阶段。两次 complex 乘法仍使用原有
binary64 FMA 表达式和舍入点，之后仍是同一个 real 乘法。非有限值 pair 使用旧的标量顺序，
发生 Span 别名时则回退到原来的三遍实现。

12 组交替配对在 1,048,576 个元素上把预热后的内核中位数从 4.280 ms 缩短到 2.976 ms，
时间减少 30.47%，吞吐为 1.438x；输出逐位一致且稳态零分配。4 组 400 帧交错配对的全部
采集表面一致，端到端墙钟中位数从 16.50 降到 16.33 秒，约改善 1.0%。
别名保护审阅后，对最终源码只补跑一次 12 组配对复查，测得 4.246 对 2.833 ms（1.4984x）。
这确认正常非别名内核保持完整，但不替代上面固定记录的完整批次测量。

长兼容性观察对两种 Exact 行为 profile 各执行一组基线/候选 1000 帧 `--threads 20` 配对。
`current` 测得 38.198 对 37.951 秒（低 0.65%），v0.4.0 测得 48.146 对 47.135 秒
（低 2.10%）。这些只是单组配对观察，不作为持续吞吐声明。候选首段/末段三分之一
工作集中位数在 `current` 为 462.4/462.9 MiB，在 v0.4.0 为 434.5/437.5 MiB：保持有界且
接近稳定，但实测有小幅上升。CPU 和单次峰值内存会随运行波动，因此不作普遍 CPU 或
内存降低声明。

每次 A/B 的亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 均一致。
另一次 12-run 候选矩阵覆盖两个 Exact profile，以及 `--threads 0`、默认 5、`1`、`5`、
`10`、`20`；另两次运行禁用全部硬件 intrinsic，并与正常 `--threads 1` 产物一致。刷新的
六路径表中 60 次 .NET 运行在每个 profile/thread 组的全部采集表面也都只有一套 hash。
Release 构建通过全部 1416 项标准 xUnit v3 测试；聚焦的 28 项测试在完全标量进程中再次通过。

### 托管 complex real-input 直接暂存

caller-buffer 托管 complex FFT 原先会先用 real double 填满整条 `Complex` 目标，再把该目标
复制到 plan 的线程本地工作值。real-input 入口现在会把 double 直接写入同一组工作值，然后
调用未改变的 FFT execute 路径。因子分解、蝶形运算及其顺序、归一化、scratch 选择和最后的
`Complex` 输出转换均未改变。

8 组正反顺序微基准每次测量运行 512 次已预热的 32K 变换，稳态分配为零。基线中位数为
437.804 us/次，候选为 424.065 us/次，缩短 3.14%；checksum 与抽样输出位完全一致。4 组
200 帧交错配对的全部采集表面一致，墙钟中位数从 9.93 降至 9.52 秒；因果结论仍采用更长门禁。

两组正反顺序的 1000 帧 Exact `current --threads 20` 配对每次均产生 2000 个有序 field。
墙钟中位数从 40.030 降至 39.448 秒（缩短 1.45%），CPU 时间中位数从 323.492 降至
317.922 秒（减少 1.72%）。基线与候选的采样峰值工作集中位数分别为 444.3 MiB 和 406.0 MiB；
单次峰值会随进程放置变化，因此这里只视为内存有界证据，不作普遍内存降低声明。

所有长短运行的亮度、色度、原始 JSON、stdout、归一化 stderr/日志和有序 `fileLoc` 均一致。
另一次 24-run 门禁覆盖 Exact/IPP-fast、两个行为 profile、`--threads 0`、默认 5 和
`--threads 20`；另外 4 次运行覆盖默认、禁 FMA、禁 AVX 和完全标量硬件 intrinsic 配置。
全部表面一致。原生桥接 smoke 仅依赖系统 DLL，全部 1403 项标准 xUnit v3 测试均通过。

### 托管 real-FFT 原地输入

紧凑 VHS block 不会向外暴露原始解调输出。在 sharpness、chroma trap 和可选视频裁剪选定
最终输入后，托管 Exact real FFT 现在会原地消费这条由当前阶段独占的数组。运算、打包、
频谱写入顺序、滤波器和逆变换均未改变。诊断/原始输出调用仍使用复制 API，IPP-fast 仍
调用同一个原生前向 FFT。

12 组交替微基准每个测量 batch 都只对 512 条独立 32K 输入各变换一次；原地路径赢下
11 组，中位数从 54.953 降至 53.732 ms（缩短 2.22%）。4 组 160 帧端到端配对在吞吐上
保持中性。持续门禁采用两组正反顺序的 1000 帧 Exact `current --threads 20` 配对：合计
墙钟从 79.259 降至 78.562 秒（缩短 0.88%，吞吐 1.0089x），两组都由候选获胜。合计
CPU 时间增加 0.63%，因此不声明 CPU 效率提升。

四次长跑均产生 2000 个有序 field；亮度、色度、原始 JSON、stdout、归一化 stderr/日志和
有序 `fileLoc` 全部一致。候选两次运行的首段/末段三分之一工作集中位数分别为
441.6/443.7 MiB 与 579.7/563.4 MiB。峰值会随进程放置明显波动，因此不声明内存降低；
两次运行都没有持续增长。另一次 24-run 门禁覆盖 Exact/IPP-fast、两个行为 profile 以及
`--threads 0`、默认 5 和 `--threads 20`，所有采集表面与基线一致，并且跨线程保持确定。
原生桥接 smoke 仅依赖系统 DLL；全部 1402 项 xUnit v3 测试均通过，IPP 用例实际执行而非
跳过。

### 托管 AVX real radix-4 FFT

Exact real-FFT 的前向和逆向 radix-4 阶段现在每个 AVX 块处理两个彼此独立的复数点。
每个 lane 仍保持原标量乘、加、减顺序，不使用 FMA；标量尾部和原有无 AVX 路径均保留。
覆盖长度 4 至 131072 的 96 组前向/逆向位模式与基线逐字节一致。JIT 反汇编包含独立的
`vmulpd`、`vaddpd` 和 `vsubpd`，没有融合乘加指令。

6 组正反顺序微基准在长度 131072、每次 3000 组前向/逆向变换下，把墙钟中位数从
3.7935 降到 3.6182 秒（缩短 4.62%，吞吐 1.0485x）。4 组 160 帧 Exact
`current --threads 20` 配对把墙钟中位数缩短 4.27%、CPU 时间减少 12.49%；对应的
v0.4.0 与 IPP-fast 检查保持中性或略快。每次运行的九项兼容表面全部一致。上方独立的
30 次矩阵用于公开绝对时间，不会把因果 A/B 的百分比直接套到每一个单元格。

两组正反顺序 1000 帧配对更接近中性：合计墙钟从 80.222 降到 79.549 秒（缩短 0.84%），
合计 CPU 时间从 651.172 增到 658.203 秒（增加 1.08%）。因此 160 帧结果是短窗口热点
证据，不能理解成持续吞吐也会提高相同百分比。

一次 1000 帧采样运行完成了 2000 个 field，亮度、色度与 JSON hash 均一致。峰值工作集
为 415.3 MiB；末三分之一中位数为 409.9 MiB，仅比首三分之一高 2.3 MiB，因此没有引入
随时间增长的内存占用。标准 xUnit v3 套件共有 1401 项通过测试，其中 4 项固定了
radix-4 hash，并且在禁用 AVX 后同样通过。

上一版 40 帧表让固定启动成本占比过大，尤其会放大 Python 的启动成本。例如默认
IPP-fast `current` 在窗口扩到 160 帧后从 6.351x 变为 5.450x：Python 只从 19.791
增至 53.288 秒，而 .NET 从 3.116 增至 9.778 秒。这是启动成本被摊薄，不是候选发生
性能回退。更早的 NTSC Betamax HiFi 表使用另一份私有夹具，完全不能直接比较。后续
刷新将固定使用 160 帧、三轮重排方法；性能因果结论仍以更长的匹配 A/B 为准。

全部 60 次 .NET 矩阵运行在所有采集面上保持确定；已合并 Python PR341 的 15 次运行
也保持确定。Python v0.4.0 的 15 次运行产生了 15 套不同的亮度、色度、
...
32 位 sortable prefix，不再使用两段 16 位阶段。最终候选 prefix、按源顺序过滤、
Quickselect 输入、浮点表达式和 detector 状态都不变；串行路径也不变。每个 worker 私有
histogram 的最大存储量从 512 KiB 降至 16 KiB；在 detector 内部 4-worker 上限下，
总量从 2 MiB 降至 64 KiB。

IPP-fast 明确保留原来的 16+16 位路径。最初让两个后端都走紧凑路径时，IPP 四组配对的
墙钟反而增加 1.6%，CPU 时间增加 1.8%。最终只把 Exact 路由到紧凑阶段后，IPP 四组
配对为中性结果：墙钟缩短 1.31%，CPU 时间约增加 0.4%；IPP 合同内的基线/候选产物
完全一致。

固定生产尺寸的选择 probe 中，紧凑实现八组配对全部获胜，中位墙钟从 0.966511 降至
0.891624 秒（缩短 7.75%，吞吐约 1.084x），checksum
`000000002704317B` 不变。这是热点证据，不作为端到端结论。四组交错的 160 帧 Exact
配对将平衡后的总墙钟从 34.113 降至 33.114 秒（缩短 2.93%），CPU 时间从 303.11
降至 278.11 秒（减少 8.25%）；逐组只有两组获胜，所以短测仍有噪声。

端到端因果门禁为两组正反顺序的 1000 帧 Exact `current --threads 20` 配对。合计墙钟
从 82.260 降至 80.387 秒（缩短 2.28%，吞吐为 1.0233x），CPU 时间从 644.594
降至 640.391 秒（减少 0.65%）。候选在每次运行中的工作集采样保持有界；受进程放置
波动影响，观测峰值为 387.5 至 418.7 MiB，未见持续增长或 OOM 行为。

最终严格门禁共 44 次基线/候选运行：八组短交错配对、两组正反顺序长配对，以及 24 次
Exact/IPP-fast 线程运行，覆盖两个行为 profile 的 `--threads 0`、默认 5 和
`--threads 20`。亮度、色度、原始 JSON、有序 `fileLoc`、stdout、归一化 stderr 和
归一化日志全部一致。detector 聚焦测试 25/25 通过；全部 1382 项 xUnit v3 测试在原生
硬件、禁用 AVX2 和禁用全部硬件 intrinsic 三种模式下均通过。Release solution build
为零 warning、零 error。

### 连续 AVX2 VSync radix histogram 合并

这个已合并的历史候选基于 main `845d8d1`，其可执行文件 SHA-256 为
...
double 精度 radix-8 内核现在用托管 AVX 每次计算两个互相独立的蝶形。每个 lane 保留
原有加法、减法、乘法与转换顺序，不使用 FMA，也不重结合归约；奇数尾部以及不支持
AVX 的主机继续执行原标量路径。

两组反序 1000 帧 Exact 配对将 v0.4.0 从 64.106 降至 63.742 秒（缩短 0.57%），
CPU 时间从 342.547 降至 336.578 秒（减少 1.74%）；`current` 从 52.405 降至
52.259 秒（缩短 0.28%），CPU 时间从 392.438 降至 374.859 秒（减少 4.48%）。
两个 profile 的候选采样峰值工作集都约为 778 MiB。短窗口矩阵用于报告当前吞吐快照；
这些直接比较 main/候选的长配对才是本次改动的因果性能证据。

全部 1349 项 xUnit v3 测试在正常模式和禁用 AVX 时均通过。另有 24 次严格基线/候选
运行覆盖 Exact、IPP-fast、两个 profile，以及 `--threads 0`、默认 5 和
`--threads 20`；亮度、色度、原始 JSON、stdout、耗时归一化 stderr、时间戳归一化
日志、有序 `fileLoc` 和跨线程确定性全部一致。60 次矩阵所用可执行文件的 SHA-256 为
...
托管 `current` CTI 距离阶段现在用 AVX/FMA 一次计算 8 个独立 float32 lane。每条 lane
仍保留原有减法、乘法、融合乘加、平方根、阈值、倒数、权重和写入顺序；向量尾部以及
不支持 AVX/FMA 的主机继续执行原标量表达式。18 项固定 PR341 xUnit v3 用例在硬件
路径及单独禁用 AVX 的进程中都通过。六对交错的实际尺寸内核测试保持同一个 SHA-256，
墙钟中位数从 4969.497 降至 4387.421 ms（缩短 11.71%），CPU 中位数从 4812.500
降至 4289.063 ms（减少 10.88%）。

两组反序 1000 帧 Exact `current --threads 20` 配对基本中性：基线/候选墙钟中位数为
50.687/50.793 秒（-0.21%），CPU 为 378.680/381.266 秒，因此不声称 Exact 端到端
提速。两组相同规格的 IPP-fast 配对都由候选胜出，墙钟中位数从 43.730 降至
42.872 秒（缩短 1.96%），CPU 从 255.273 降至 251.797 秒（减少 1.36%）；最大采样
工作集在这些 IPP 配对中为 736.8/734.6 MiB。8 次测量的亮度、色度、原始 JSON、
有序 `fileLoc`、stdout、
归一化 stderr 和时间戳归一化日志全部匹配。刷新后的 60 次矩阵全部确定，并在默认 5、
`--threads 1/5/10/20` 下为每个 backend/profile 只产生一套 hash。
...
### Complex32 第二阶段 packet 原地变换

Exact float32 PocketFFT 的大变换布局在第二阶段已经独占互不重叠且连续的 source packet。
现在直接原地变换各 source packet，再把结果 scatter 到独立 destination buffer；不再从
堆上分配或从 `ArrayPool<Complex32>` 租用临时 packet，也不再先完整复制 source packet。并行 packet 不会
...
有序 `fileLoc`、场数和退出行为上全部一致。墙钟中位数从 11.59 降至 11.27 秒
（低 2.8%），吞吐提升 2.8%，CPU 时间下降 1.9%，候选赢得 5 组。4 组单 worker 配对
从 69.56 降至 68.98 秒（低 0.8%），CPU 时间和峰值内存基本持平。

3 组 1000 帧、20 workers 配对继续完全一致，但胜负随主机调度顺序变化。其配对墙钟
中位变化为 +0.14%，配对 CPU 中位变化为 +1.18%，因此明确将长跑吞吐和 CPU 效率归类
为持平。候选峰值工作集保持在约 443 MiB，没有随进度增长。

辅助采样 trace 将 `MemmoveNative` 归因从 8.391 降至 5.454 CPU 秒（低 35.0%），平均
有效核心从 7.72 升至 8.43，P90 从 9.83 升至 11.22。该 trace 的较旧基线早于
`a059580`，所以它只用于确认热点消失和多核形态；上面的精确 main 交错 A/B 才是因果
性能门禁。刷新后的 90 次矩阵覆盖全部 6 种 Python/.NET profile 以及默认、1、5、10、
20 workers。60 次 .NET 和 15 次 Python PR341 运行均保持确定性。Python v0.4.0 的
15 次运行各自产生不同的亮度、色度、JSON 和归一化日志 hash，因此严格 oracle 仍是
`g4315520 --threads 0`。
...
`PocketFftReal32.Plan` 现在会保留不可变的单位根表，供因子构造及每次复数化的
正向/逆向重组复用。`PocketFftComplex32` 则按 root length 共享不可变的
`SinCos2PiByN` 表，覆盖大型多阶段变换和带根表的 packet plan。该缓存使用 32 项
FIFO 上限；淘汰只移除缓存引用，正在运行的变换仍会安全持有其不可变表。scratch
数组、变换状态、数据类型、运算顺序和 float 转换点均未改变，root key 也与原有
plan 维度一致。

匹配的最终 200 帧 Exact `current --threads 20` trace 将 sampled allocation amount
从 579,283,536 降至 541,701,824 bytes（减少 6.49%），sampled object bytes 从
391,712,736 降至 373,960,808（减少 4.53%）。最终 bounded-cache 候选与 main
`ced6afb` 的三对 1000 帧比较结果有涨有跌，因此按吞吐中性处理：只有 1/3 对更快，
main/候选墙钟中位数为 44.924/44.985 秒（候选增加 0.14%，吞吐 0.999x），CPU 时间
中位数为 332.672/333.734 秒（增加 0.32%）。峰值工作集的中位数为 391.9/396.5 MiB；
其中一次候选采样达到 614.3 MiB，因此不根据这些短进程采样声明内存下降。每次运行的
九项兼容面均一致；最终 60 次性能矩阵和 12 次确定性矩阵也全部通过。聚焦的 16 KiB
单元测试阈值只测量热态调用线程的分配；上面的进程级分配结论来自匹配 trace，
不来自这些单元测试。

### 托管精确 VHS 阈值扫描 AVX

current VHS 的第二遍标量精确阈值扫描现在使用 ordered non-signaling 比较，在每个 AVX
步骤中分类四组相邻 double。falling/rising mask 仍按递增的标量索引顺序提交；未修改的
有效网格判断继续保持候选顺序、取模表达式、阈值和提前退出。标量尾部及无 AVX 回退
继续使用原表达式，没有引入 FMA、reduction 重排、新 worker 或常驻样本缓冲区。

聚焦的标量/AVX oracle 覆盖八种长度，并增加一组固定的 vector/tail 用例，包含
quiet/signaling NaN、无穷值、有符号零及精确 crossing 索引。34 项 detector 用例在
普通硬件上全部通过；禁用全部硬件 intrinsic 时 33 项通过，AVX-only 用例明确跳过。
在匹配的 400 帧
IPP-fast trace 中，`VhsSyncDetector.DetectFiltered` 的 sampled CPU 从 1263.307 降至
1173.588 ms（减少 7.1%）。四组正反顺序的 400 帧配对，基线/候选墙钟中位数为
16.98/16.47 秒；保守的配对差值收益为 1.3%，因此不把更大的未配对差距当作因果收益。

两组正反顺序的 1000 帧 IPP-fast `current --threads 20` 配对将墙钟中位数从 36.341
降至 35.822 秒（缩短 1.43%，吞吐 1.0145x），进程 CPU 时间中位数从 222.078 降至
220.789 秒（减少 0.58%），有效核心数中位数从 6.11 变为 6.16。四次 2000-field
...
.NET v0.4.0 列继续沿用此前同主机测量；该检查点上方两个 current 列使用当时候选的
单文件可执行文件。

### AVX2 Hilbert 实数频谱缩放

VHS/LD 解析信号准备阶段会让每个 double 精度复数频谱值乘以一个实数 Hilbert 乘数。
新的托管 AVX2 路径一次载入 4 个乘数，把它们复制到对应的实部/虚部 lane，再执行两次
彼此独立的 4-double 乘法。这里没有 FMA、归约或表达式重排。只要组内任一复数分量或
...
### current 色度 burst 拟合器循环融合

显式启用的 `current` 色度 burst 最小二乘拟合器现在用一个按索引递增的循环准备
cosine、sine、residual 和 Jacobian，不再分成五遍；四个非向量 dot sum 也按原先完全
相同的递增顺序在该循环中累加，同时移除了临时 theta 和 sine buffer。数据类型、逐样本表达式、
转换点、兼容 OpenBLAS 的 dot 归约、求解器顺序、worker 边界和状态转换均未改变，
也没有引入 FMA 或重结合。

九组交替进程测试各执行 100,000 次完整 `Fit`，中位数从 1,625.603 降到
1,489.642 ms（缩短 8.36%，吞吐 1.091x）。每次运行都保持同一 checksum 和
2,080-byte 进程级分配计数；这是隔离内核证据，不是端到端结论。

在同一个本地私有 40 MHz PAL VHS 样本上，三组交错的 1,000 帧 Exact
`current --threads 20` 配对数值上都由候选获胜。平均墙钟从 39.572 降到 39.416 秒
（缩短 0.39%），但配对差值的 95% 区间为 -0.011 至 0.322 秒，仍包含零；平均进程
CPU 时间也从 322.464 增至 326.474 秒（增加 1.24%）。因此端到端吞吐按中性处理，
也不声明 CPU 效率提升。
采样峰值工作集保持在此前确认的 741 MiB 边界以内，也没有新增无界分配路径；受噪声
影响的样本不支持常驻内存下降结论。

六次配对长运行和一次最终源码确认运行的亮度、色度、原始 JSON、stdout、耗时归一化
stderr、时间戳归一化日志和全部 2,000 个有序 `fileLoc` 完全一致。另有 12 次短门禁覆盖 Exact v0.4.0/`current`
的显式零、默认 5 和 20 workers，全部基线/候选及跨线程兼容面均精确一致。三个固定的
拟合器测试在普通模式和禁用 AVX 时都通过；完整 Release 套件为 1,397 项通过，另有
4 项仅因本机构建目录没有 IPP runtime 而跳过。首页表格仍保留上一次完整五路径矩阵，
因为把这组不同的 1,000 帧窗口直接代入会让包含启动时间的倍率失去可比性。
...
七组交错、每组 10,000 次的同步微基准中，中位墙钟从 264.306 ms 降至
251.985 ms（4.66%），分配从 739,600,040 降至 401,680,040 bytes（45.69%），
所有运行只有一个 checksum。三组重排顺序的 1,000 帧 Exact
`current --threads 20` 配对受调度噪声影响，因此端到端吞吐按中性处理。色度相位的
五组 100,000 次微基准将分配中位数从 6,547,233,136 降至 5,881,975,184 bytes
（10.16%）；候选五组中胜三组，配对差值中位数的墙钟下降 1.23%。

两组顺序相反的 1,000 帧色度配对均由候选胜出，合计墙钟从 79.645 降至
79.056 秒（下降 0.74%），合计 CPU 时间上升 0.43%，因此不宣称 CPU 效率提升。
每次长跑的亮度、色度、原始 JSON、stdout、归一化 stderr/日志以及全部 2,000 个有序
`fileLoc` 均一致。一段支持性的 12 秒固定时长稳态 trace 将采样分配量从
247,776,232 降至 228,539,336 bytes（7.76%），原有的 classified-pulse
`AddWithResize` leaf 也从主要热点中消失；但 trace 窗口并未按完全相同的 field 数归一化，
所以该百分比不作为完整流水线分配结论。

更大范围的同步 scratch workspace 实验虽然让微基准分配下降 36.62%，但两组反向顺序
真实配对分别慢 1.02% 和 1.74%，因此已完整回退，没有用吞吐换取更漂亮的单项分配数字。
这些同版本小幅结果不足以替代完整五档 profile 刷新，所以首页 Python 对比矩阵保持不变。
...
273.375 变为 275.297 秒（+0.70%），有效核心数从 9.24 增至 9.60（+3.87%）。六次
长运行的亮度 TBC、色度 TBC、原始 JSON、stdout、耗时归一化 stderr、时间戳归一化
日志和全部有序 `fileLoc` 均完全一致。

另一组六次门禁覆盖未改变的 `--threads 0` 路径、省略参数的默认五 worker，以及
`--threads 20`。基线和候选 hash 一致，三个 worker 模式也都保持确定性。刷新的
60 次 Exact/IPP-fast、v0.4.0/`current`、默认/1/5/10/20-worker 矩阵，在每个单元格的
七项采集表面上都只产生一套 hash。SOS workspace 聚焦测试通过 45/45，current 色度
并行测试通过 10/10；标准 xUnit v3 套件的 1,448 项测试全部通过。
...
dotnet test --solution VHSDecodeDotNet.slnx -c Release --no-build --no-restore --minimum-expected-tests 1615
...
`dotnet test` 和 Visual Studio Test Explorer 暴露 **1,615** 个可独立发现的测试。
