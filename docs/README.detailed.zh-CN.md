...
直接 raw `fLaC` `.ldf`/`.flac` 输入现在只在第一个元数据块是完整 34 字节
STREAMINFO、并且声明 40 kHz、单声道、PCM16 和已知非零样本总数时使用内置
libsndfile。文件句柄按需打开；连续读取不做 seek，随机读取使用精确 frame seek，
一个池化 PCM16 workspace 继续执行不变的 `short` 到 `double` 转换。原生后端不可用
或不支持时只切换一次到既有 FFmpeg/PyAV 兼容加载器。Ogg/FLAC、立体声、PCM24、
其他采样率、未知总数、畸形文件头、`.vhs`、`.wav` 和 `raw.oga` 继续走 FFmpeg；
原生 seek/read 错误会直接报告，不会被静默切换后端掩盖。
...
- 对不属于严格门控的直接 40 kHz 单声道 PCM16 raw-FLAC 路径的容器输入，
  需要 `ffmpeg` 和 `ffprobe` 位于 `PATH`
- 直接 raw-FLAC RF 输入、默认 HiFi FLAC 输出和 LD `--write-test-ldf` 使用内置
  libsndfile，不需要 FFmpeg；各路径仍保留文档所述的 FFmpeg 回退或兼容边界
...
`dotnet test` 和 Visual Studio Test Explorer 暴露 **1,190** 个可独立发现的测试。
