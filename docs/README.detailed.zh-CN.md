...
| --- | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 14.481 s | 6.264 s / 2.312x / 56.7% | 8.050 s / 1.799x / 44.4% | 6.162 s / 2.350x / 57.4% | 7.861 s / 1.842x / 45.7% |
| `--threads 1` | 18.486 s | 19.746 s / 0.936x / 慢 6.8% | 22.281 s / 0.830x / 慢 20.5% | 19.346 s / 0.956x / 慢 4.7% | 21.503 s / 0.860x / 慢 16.3% |
...
`2b170b4`，其 `VHSDecode.Core.dll` SHA-256 为
...
的中位数为 25.831 秒，且彼此完全相同。全部 Exact v0.4.0 运行都在亮度、色度、
...
JSON，但产生了 8 种归一化日志 hash。更早的一份固定矩阵还观察到 Python 在不同
