...
| 预览 IPP 20 MSPS | 30.56（30.48-30.63） | 4.60 核 / 22.99% | 40.09% / 52.71% | 1.40% / 13% | 0.08% / 1% | 1.52% / 23% | 3,283.0 MiB（+180.3） |

紧邻测试前的 10 秒空闲基线平均为整机 CPU 5.05%、GPU SM 0%、NVENC 0%、
GPU 显存 3,102.7 MiB；表中的整机均值是原始值，没有减去这条有噪声的基线。CUDA 在
完整 40 MSPS、完整 20 MSPS 与 20 MSPS preview 中的源帧吞吐分别为 IPP 的
1.484x、1.333x、1.539x。在这份较长门禁中，完整解码从 40 降到 20 MSPS 后，CUDA
吞吐提高 1.40%，IPP 提高 12.91%。冷启动 W5 平均为 CUDA 1.158 秒、IPP 2.124 秒。
...
完整 40 MSPS A-B-B-A 在输出哈希相同的情况下测得 CUDA 62.24、IPP 24.51 源 fps。
