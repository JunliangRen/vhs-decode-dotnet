...
same private real PAL 40 MSPS `.ldf` capture (21,389,175,871 bytes,
approximately 631 seconds); its filename is intentionally not published.
...
`b893bf7` plus the current RF chroma pass optimization. Each
...
main commit `b893bf7` plus the current RF chroma pass optimization. Its product
version is `2.10.0+b893bf7d39edf574609a7abc40237a4db32eaf9f`, and its
137,284,417-byte single-file `decode.exe` SHA-256 is
...
The `current` path formerly quantized the complete double buffer to float32,
then rolled it while accumulating the DC mean in descending source order, and
finally subtracted the mean with a second float32 conversion. The retained
candidate combines the first quantization with the roll and accumulation. Its
AVX loop still adds lanes in the original `3, 2, 1, 0` order, and the final
centering loop vectorizes only independent subtraction and conversion. It does
not use FMA or reassociate the reduction. The scalar route remains available
when AVX is disabled or the mean is non-finite.

A frozen-old-implementation test covers 13 buffer lengths, nine positive,
negative, and oversized moves, the heap-backed wrapped buffer, distinct NaN
payloads, both infinities, finite signed-zero input, and both AVX-required and
AVX-disabled runs. Every output double bit matched. Over eight interleaved
microbenchmark rounds, 1,000 calls on a 32,768-sample buffer fell from 28.025 to
16.166 ms (42.317%). Three opposite-order 1,000-frame Exact `current
--threads 20` pairs matched exit status, luma, chroma, raw JSON, stdout,
normalized stderr/logs, and ordered `fileLoc`; paired median wall and CPU gains
were 0.5385% and 0.6860%. The 60-run public matrix also retained one hash per
profile/backend across all five worker settings. Memory varied within ordinary
run noise, so no memory reduction is claimed. Release build was zero-warning/
zero-error, and the standard xUnit v3 run discovered 1,615 tests, with 1,612
passing and 3 expected environment skips.
...
payload materialization. Final review also identified Python-style negative
coordinates that can wrap a 16-tap sinc read to the reusable array tail. Negative
initial locations, any final linear plan within the sinc halo, and all
quadratic/cubic interpolation now force full materialization before the read.
The final 60-run matrix produced one luma, chroma, raw JSON, stdout,
normalized-stderr, normalized-log, ordered-`fileLoc`, and field-count value for
every backend/profile across all five worker settings.

Two opposite-order Exact `current --threads 20` pairs at each of 160, 500, and
1,000 frames matched every compatibility surface. The post-review 1,000-frame
gate had combined baseline/candidate wall time of 60.344/58.614 seconds (2.87%
lower) and CPU time of 575.500/555.969 seconds (3.39% lower). Candidate peak
working sets were 407.0 and 381.7 MiB without progressive growth, so this audit
claims bounded memory rather than a memory reduction. Linux CI also exposed an
interface-enumerator allocation; indexed iteration removed it without changing
pulse classification order or thresholds. Release build was zero-warning/
zero-error, and the standard xUnit v3 run discovered 1,614 tests, with 1,611
passing and 3 expected environment skips.
...
The retained candidate combines construction and exponential smoothing of the
linear TBC level-adjust array into one ascending write pass. It no longer fills
the complete raw factor array and then scans it a second time. Median/MAD
outlier clamping, the exact `Math.FusedMultiplyAdd` expression, per-sample
evaluation order, double-precision conversion points, line boundaries, and
worker ownership are unchanged. Smoothing-disabled calls retain the original
`Array.Fill` path.

The escalation gate used three interleaved 160-frame pairs, two 500-frame
pairs, and two opposite-order 1,000-frame Exact `current --threads 20` pairs.
Every run matched exit status, luma, chroma, raw JSON, stdout, normalized
stderr/log, and every ordered `fileLoc`. At 1,000 frames, both pairs favored the
candidate: median wall time moved from 29.758 to 28.799 seconds (3.22% lower),
and median process CPU time moved from 287.672 to 281.703 seconds (2.05% lower).

A candidate-only 2,000-frame run completed in 54.408 seconds at 9.82 effective
cores. Peak private memory was 588.6 MiB, below the 637.7 MiB maximum observed
in the 1,000-frame candidate gate, and the sampled timeline contained four
drops greater than 1 MiB. This establishes bounded behavior for the measured
window, not a general resident-memory reduction claim.

Explicit-zero and omitted-default-five runs were internally deterministic and
matched the 20-worker candidate output on all seven captured surfaces. The
refreshed 60-run Exact/IPP-fast, v0.4.0/`current`, default/1/5/10/20 matrix also
...
### Owned-buffer complex analytic inverse

The retained Exact candidate removes the preserving copy before the complex
analytic inverse FFT. Once Hilbert and RF filters have finished in-place, the
prepared spectrum is dead as an input artifact, so `PocketFftComplex.InverseOwned`
uses that array and the second worker-owned array as alternating pass storage.
It returns whichever array owns the final pass; the VHS workspace then keeps
that array as `FullAnalytic` and the other as reusable `DiffedAnalytic` scratch.
No array is added or retained. PocketFFT factorization, butterflies, twiddles,
normalization, AVX eligibility, floating-point types, and evaluation order are
unchanged.

The escalation gate used three interleaved 160-frame pairs, two 500-frame pairs,
and two opposite-order 1,000-frame Exact `current --threads 20` pairs. Exit
status, luma, chroma, raw JSON, stdout, normalized stderr/logs, and every ordered
`fileLoc` matched in every pair. At 1,000 frames, both pairs favored the
candidate: median wall time moved from 28.804 to 28.549 seconds (0.89% lower).
Median process CPU time was effectively unchanged at 283.367 versus 283.320
seconds (0.02% lower), consistent with removing a critical-path memory copy
rather than arithmetic.

An initial baseline-first `--threads 0` probe was noisy, so the fixed confirmation
stopped after two opposite-order pairs. Its 34.305/34.269-second medians were
neutral, all seven surfaces matched, and peak private memory was slightly lower
for the candidate. Focused storage and parallel-RF tests passed normally, with
AVX disabled, and with all hardware Intrinsics disabled.

A candidate-only 2,000-frame run completed in 54.184 seconds at 9.80 effective
cores. Peak private memory was 643.3 MiB; the timeline contained four drops over
1 MiB and stayed within 634.4-634.7 MiB for its final six seconds. This is a
bounded-window observation, not a general memory-reduction claim. The refreshed
60-run Exact/IPP-fast, v0.4.0/`current`, default/1/5/10/20 matrix retained one
hash set in every cell and across worker settings. The standard xUnit v3 suite
found 1,613 tests: 1,610 passed and 3 expected environment-dependent tests
skipped.

### AVX real-to-complex FFT input staging

The retained candidate replaces the scalar real-input staging loop in the
complex PocketFFT path with a four-sample AVX interleave. Loads, unpack
operations, and 128-bit lane permutations write the exact same
`[real, +0.0]` component sequence. No floating-point arithmetic, FMA,
reduction, FFT factor, butterfly, twiddle, normalization, or evaluation order
changes. Length-two and trailing samples use the original scalar expression;
non-AVX machines take the complete scalar path. Overlapping caller storage is
still redirected through the existing plan scratch before staging.

An eight-trial 32,768-point complete-FFT probe produced identical SHA-256
outputs and reduced the median from 219.493 to 216.729 ms (1.26%). The real-RF
gate then used three interleaved 160-frame pairs, two opposite-order 500-frame
pairs, and two opposite-order 1,000-frame Exact `current --threads 20` pairs.
All seven surfaces matched at every length. The two 1,000-frame pairs had
combined baseline/candidate wall medians of 34.711/33.171 seconds (4.44% lower)
and CPU medians of 294.758/290.016 seconds (1.61% lower). One individual wall
pair favored the baseline by 1.27%, so the retained conclusion is a small
aggregate gain under ordinary scheduler variation.

Candidate-only runs at explicit zero, omitted default-five, and 20 workers
produced the same seven hashes, both within each mode and across modes. A
2,000-frame run completed in 60.957 seconds at 8.94 effective cores. Peak
private memory was 367.7 MiB, the second-half range was 365.8-367.5 MiB, and
three drops exceeded 1 MiB. The refreshed 60-run Exact/IPP-fast matrix remained
deterministic within every cell and across worker settings. A separate
AVX-required invocation prevents the native staging check from silently taking
the scalar path; the five focused storage tests also passed with AVX disabled
and with all hardware Intrinsics disabled. The standard xUnit v3 suite found
1,613 tests, with 1,610 passes and 3 expected environment-dependent skips.

### Direct-output four-section managed SOS

The retained candidate targets the common four-section managed float32 SOS
path. It writes the center samples directly into the caller's destination and
keeps only the odd-extension edges as scratch. Coefficient conversion, initial
conditions, forward and backward section order, per-sample expression order,
padding, and IPP routing are unchanged; other section counts retain the prior
widened-buffer path.

The causal gate compared locally rebuilt clean-main and candidate binaries from
the same commit, SDK, machine, and `FolderProfile`. Three interleaved 1,000-frame
Exact `current --threads 20` pairs matched luma, chroma, raw JSON, stdout,
normalized stderr/log, and every ordered `fileLoc`. Median wall time moved from
29.63 to 28.96 seconds (2.3% lower), and CPU time moved from 281.06 to 276.69
seconds (1.6% lower). Candidate CPU time was lower in all three pairs; wall time
was lower in two of three. The warm direct call allocates fewer than 64 managed
bytes and uses no queue or input-length retained workspace.

The refreshed 60-run default/1/5/10/20 Exact/IPP-fast matrix retained one hash
for every captured surface in every cell. The focused 46-test DSP suite passed
with native intrinsics and with all hardware intrinsics disabled; the standard
xUnit v3 suite passed 1,607 tests with 3 expected environment skips.

### Rebalanced current VHS inverse scheduling

A fresh 320-frame Exact `current --threads 20` CPU trace showed about 36.6
seconds of active RF-block work against 14.2 seconds in the inverse companion
lane. The previous 12 outer workers plus 8 dedicated companions therefore left
the companion lane lightly occupied. The retained policy uses 14 outer workers
and 6 companions at 20 requested workers. The 28-block lookahead and every
existing cache, output, and workspace bound are unchanged. v0.4.0, fewer than
20 workers, GNRC, and sharpness retain their previous scheduling.

This changes only which bounded worker owns an already independent inverse.
FFT plans, buffers, arithmetic, exception priority, cross-field state, output,
metadata, and diagnostics are unchanged. Two opposite-order 1,000-frame Exact
`current` pairs matched luma, chroma, raw JSON, stdout, normalized stderr/log,
and every ordered `fileLoc`. Combined medians moved from 32.962 to 32.699
seconds (0.80% less wall time) and from 305.711 to 296.094 CPU seconds (3.15%
less). Effective cores moved from 9.27 to 9.06 because CPU work fell faster than
wall time; peak working set remained bounded at roughly 395-397 MiB.

Four interleaved 160-frame IPP-fast `current` pairs matched the same seven
surfaces and moved median wall time from 8.752 to 8.584 seconds (1.92% less),
with effectively neutral CPU time and effective cores rising from 5.85 to 5.95.
Separate v0.4.0 Exact and IPP-fast pairs were neutral. The refreshed 60-run
default/1/5/10/20 matrix retained one hash for every surface in every cell.
Release tests passed with native intrinsics (1,607 passed, 3 environment skips),
AVX2 disabled (1,607/3), and all hardware intrinsics disabled (1,606/4).
...
project exposes **1,615** independently discoverable tests to both
