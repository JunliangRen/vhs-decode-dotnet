...
最初の 5 行の直前 10-second idle baseline は system CPU 5.05%、GPU SM 0%、NVENC 0%、
GPU FB 3,102.7 MiB でした。別途修正した IPP preview 行は system CPU 6.85%、GPU FB
3,109.9 MiB の baseline を使用し、+182.1 MiB はこれを基準にしています。表の system
mean はいずれも noisy baseline を差し引かない raw value です。
CUDA の IPP 比 source throughput は full 40 MSPS で 1.484x、full 20 MSPS で 1.333x、
20 MSPS preview で 1.370x です。この長い gate で full decode を 40 から 20 MSPS にすると、
CUDA throughput は 1.40%、IPP は 12.91% 上がりました。cold W5 平均は CUDA 1.158 秒、
IPP 1.913 秒です。full decode は video encode を行わないため NVENC は zero です。CUDA
preview は block-linear NV12/NVENC、IPP preview は `NVENC + CUDA YADIF x2` を使用しました。
各 configuration は、それぞれの backend/build 内で repeatable output hash を保持しました。
これは当該 capture、machine、driver、測定した build に限定した結果で、cross-hardware
または quality-equivalence guarantee ではありません。

full/CUDA の 5 行は commit
`41bfd923e2a47f926db3d4ea3c3ff70adbecaa48`、executable SHA-256
`EF07E96D224A4BBEADBDE44EAEDB439C50D1646FA173FD57479255367B9A8868` で測定し、raw
CSV、log、hash、100 ms sample を
`.artifacts/current-resource-matrix-20260820-192956/` に保持しています。この run の元の
IPP preview 行は、20 MSPS window coordinate を二重に 2 で割っていたため無効です。
修正した IPP 行は commit `1fb1455ec6feceeb5c3a95f882b01ebbac61306c`、executable
SHA-256 `CE054C2CD9EA1793080F39121C2AA154130FDF5F75C14F07667DD3F214435C6F` で再測定し、
`.artifacts/current-resource-matrix-20260820-200449/` に保持しています。20-window run
2 回の aggregate hash はともに
`C1A91E1709BA857843F0790BC61B42BB7C8AACD3A95E28804AF29E2ECDAD1A30` でした。

同日の session 間で CUDA absolute throughput は安定しませんでした。この sustained
matrix 直前の別の fresh-build full-40 A-B-B-A は CUDA 62.24、IPP 24.51 source fps を
測定し、hash は各 backend/build configuration 内で repeatable でした。current toolchain
で最初の `d913457` CUDA 実装を
...
local RTX 4070 と real 631.452-second PAL 40 MSPS capture 1 本で、修正済み sustained
matrix は backend ごとに 20-window run を 2 回行い、CUDA 47.032、IPP 34.332 source
fps、つまり 1.370x を測定しました。cold W5 平均は CUDA 1.158 秒、IPP 1.913 秒です。
両 output は H.264 Main、768x576、progressive 50 fps で期待する PAL colour tag を
保持しました。

final-source quality recheck は matrix と同じ順序で W5/W15/W20/W25/W30/W35 を request
しました。persistent encoder に同じ request sequence を与えるため、W20 は集計しない
bridge としました。1 field alignment のため IPP の最初の output frame を除き、
W5/W15/W25/W30/W35 を比較しました。default CUDA preview の rendered SSIM
Y/U/V/All 平均は 0.914657/0.957361/0.966698/0.930448 でした。full-decode line-phase
guard を強制すると 0.910581/0.954161/0.963665/0.926692、cross-field dropout と chroma
stabilization を無効化すると 0.904516/0.952275/0.963803/0.922357 でした。default の
combined SSIM は全 window で両 alternative より高く、修正済み coordinate でも 2 つの
preview-only choice を再検証しました。結果は
`.artifacts/cuda-preview-current-validation-20260820-203748/` に保持しています。
...
全 60 回の .NET 測定は 2026-08-26 に main commit `b893bf7` と current RF chroma pass
optimization を含む同じ self-contained .NET 11 Preview 7 candidate でまとめて更新しました。
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| default（5） | 52.811 s | 54.243 s | 11.598 s / 4.553x / 78.04% | 11.398 s / 4.759x / 78.99% | 10.230 s / 5.162x / 80.63% | 7.997 s / 6.783x / 85.26% |
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| default（5） | 52.583-62.222 s | 53.893-58.195 s | 11.495-11.834 s | 10.811-11.412 s | 10.217-10.272 s | 7.899-8.057 s |
...
全 20 個の .NET cell は、同じ .NET 11 Preview 7 candidate による 2026-08-26 の
...
更新した全 .NET cell は main commit `b893bf7` と current RF chroma pass optimization を含む
self-contained Preview 7 candidate を使いました。product version は
`2.10.0+b893bf7d39edf574609a7abc40237a4db32eaf9f`、137,284,417-byte single-file
`decode.exe` の SHA-256 は
`92481E633B4CD5C80DDD1CD6C26AD5D045B9046763CD8B1F84083F8EFBD73303` です。
...
### Current RF chroma quantize/shift fusion

`current` path は以前、double buffer 全体を float32 に quantize してから、source index の
降順で DC mean を accumulate しながら roll し、最後に mean を引いて 2 回目の float32
conversion を行っていました。retained candidate は最初の quantization を roll と
accumulation に融合します。AVX loop は従来どおり `3, 2, 1, 0` lane 順で加算し、最後の
centering loop は独立した subtraction/conversion だけを vectorize します。FMA も reduction
の reassociation も行いません。AVX disabled または mean が non-finite の場合は scalar route
を維持します。

frozen old implementation test は 13 buffer length、9 positive/negative/oversized move、
heap-backed wrapped buffer、distinct NaN payload、両 infinity、finite mean の signed zero、
AVX-required/AVX-disabled run をカバーし、すべての output double bit が一致しました。
8 interleaved microbenchmark round で、32,768-sample buffer の 1,000 call は
28.025 から 16.166 ms へ 42.317% 短縮しました。opposite-order の 1,000-frame Exact
`current --threads 20` 3 pair は exit status、luma、chroma、raw JSON、stdout、normalized
stderr/log、ordered `fileLoc` が一致し、paired median wall/CPU gain は 0.5385%/0.6860%
でした。60-run public matrix も 5 worker setting の各 profile/backend で 1 hash を維持しました。
memory は通常の run noise 内で変動したため、削減は主張しません。Release build は
0 warning/0 error、standard xUnit v3 は 1,615 tests を discover し、1,612 passed、expected
...
`Pass8FirstIndex` でした。隣接する vector overload と in-place helper には既に
`AggressiveInlining` があり、この candidate は欠けていた scalar helper に同じ hint を
追加するだけです。add/subtract とその順序、data type、FFT plan、buffer ownership、
worker policy、exception behavior は変更しません。

Exact `current --threads 20` は 160、500、1,000 frames で各 3 interleaved pair を完了し、
exit code、luma、chroma、raw JSON、stdout、normalized stderr/log、ordered `fileLoc` が
一致しました。1,000-frame gate の baseline/candidate 独立 median は wall time が
30.213/29.576 秒、process CPU time が 290.719/279.297 秒でした。3 pair の改善率 median は
wall 1.35%、CPU 3.93% で、baseline/candidate peak working set は 425.4/388.6 MiB でした。
別の explicit-zero 3-pair run の baseline/candidate 独立 wall median は 37.492/36.962 秒、
pair ごとの改善率 median は 0.43% で、high-worker gain による low-worker regression は
測定されませんでした。

profile/thread gate は `v0.4.0` と `current` の option 省略、explicit zero、20 workers を
網羅しました。更新した 60-run Exact/IPP-fast matrix は各 cell 内および
default/1/5/10/20 workers 間で deterministic でした。Release solution は 0 warning、
0 error で build され、standard xUnit v3 は 1,613 tests を discover し、1,610 passed、
expected environment skip は 3 でした。
...
### Complex analytic inverse の owned-buffer 化

採用した Exact candidate は、complex analytic inverse FFT 前の preserving copy を除きます。
Hilbert/RF filter の in-place 処理後は prepared spectrum を input artifact として保持する
必要がないため、`PocketFftComplex.InverseOwned` はその array と 2 本目の worker-owned
array を alternating pass storage にします。final pass を所有する array を返し、VHS
workspace はそれを `FullAnalytic`、残りを reusable `DiffedAnalytic` scratch として扱います。
array の追加や余分な保持はありません。PocketFFT factorization、butterfly、twiddle、
normalization、AVX eligibility、floating-point type、evaluation order は変更しません。

段階 gate は interleaved 160-frame 3 pair、500-frame 2 pair、実行順を反転した 1,000-frame
Exact `current --threads 20` 2 pair を使いました。すべての pair で exit status、luma、
chroma、raw JSON、stdout、normalized stderr/log、全 ordered `fileLoc` が一致しました。
1,000-frame は 2 pair とも candidate が勝ち、median wall time は 28.804 から 28.549 秒へ
0.89% 減りました。median process CPU time は 283.367 対 283.320 秒（0.02% 減）で
実質同等であり、arithmetic ではなく critical-path memory copy を除いた結果と整合します。

最初の baseline-first `--threads 0` probe は noisy だったため、fixed confirmation は実行順を
反転した 2 pair で打ち切りました。median は 34.305/34.269 秒で neutral、7 surface はすべて
一致し、candidate の peak private memory はわずかに低くなりました。focused storage/RF test
は通常時、AVX 無効時、all hardware Intrinsics 無効時に成功しました。

candidate-only 2,000-frame run は 54.184 秒、9.80 effective cores で完了しました。peak private
memory は 643.3 MiB、1 MiB を超える drop は 4 回で、最後の 6 秒は 634.4-634.7 MiB に
収まりました。これは bounded measurement window の観測であり、一般的な memory reduction
claim ではありません。更新した 60-run Exact/IPP-fast、v0.4.0/`current`、
default/1/5/10/20 matrix は各 cell と worker setting 間で 1 hash set を維持しました。
standard xUnit v3 suite は 1,613 tests を discover し、1,610 passed、expected environment
skip は 3 でした。

### AVX real-to-complex FFT input staging

採用した candidate は complex PocketFFT path の scalar real-input staging loop を、
4 sample ごとの AVX interleave に置き換えます。load、unpack、128-bit lane permutation は
従来と完全に同じ `[real, +0.0]` component sequence を書きます。floating-point
arithmetic、FMA、reduction、FFT factor、butterfly、twiddle、normalization、evaluation
order は変更しません。length 2 と trailing sample は元の scalar expression を使い、
非 AVX machine は完全な scalar path を通ります。caller storage が overlap する場合も、
staging 前に既存の plan scratch へ redirect する挙動を維持します。

8-trial の 32,768-point complete-FFT probe は同一 SHA-256 を生成し、median は
219.493 から 216.729 ms（1.26% 減）へ移りました。real-RF gate は interleaved
160-frame 3 pair、実行順を反転した 500-frame 2 pair と 1,000-frame Exact
`current --threads 20` 2 pair を使い、全 length の 7 surface が一致しました。
1,000-frame 2 pair の baseline/candidate combined wall median は 34.711/33.171 秒
（4.44% 減）、CPU median は 294.758/290.016 秒（1.61% 減）でした。wall time の
1 pair は baseline が 1.27% 速かったため、通常の scheduler variation 下における
小さな aggregate gain として結論します。

candidate-only の explicit zero、option 省略 default-five、20-worker run は mode 内と
mode 間で同じ 7 hash を生成しました。2,000-frame run は 60.957 秒、8.94 effective
cores で完了し、peak private memory は 367.7 MiB、second-half range は
365.8-367.5 MiB、1 MiB を超える drop は 3 回でした。更新した 60-run
Exact/IPP-fast matrix は各 cell と worker setting 間で deterministic でした。独立した
AVX-required invocation により native staging check が scalar path へ silent fallback
できないようにし、focused storage 5 tests は AVX 無効と all hardware Intrinsics 無効でも
成功しました。standard xUnit v3 suite は 1,613 tests を discover、1,610 passed、
expected environment-dependent skip は 3 でした。

### 4-section managed SOS の direct output

今回採用した candidate は、よく使われる 4-section managed float32 SOS path で center sample を
caller の destination へ直接書き、odd extension の左右 edge だけを scratch に保持します。
coefficient conversion、initial condition、forward/backward section order、sample ごとの expression
order、padding、IPP routing は変更しません。他の section count は従来の extended-buffer path を
そのまま使います。

因果 gate は、同じ commit、SDK、machine、`FolderProfile` で作った local clean-main binary と
candidate binary を使いました。interleaved 1,000-frame Exact `current --threads 20` 3 pair は、
luma、chroma、raw JSON、stdout、normalized stderr/log、すべての ordered `fileLoc` で一致しました。
wall-time median は 29.63 から 28.96 秒へ 2.3% 減り、CPU-time median は 281.06 から
276.69 秒へ 1.6% 減りました。candidate の CPU time は 3 pair すべてで低く、wall time は
2 pair で勝ち、1 pair で負けました。direct hot call の managed allocation は 64 bytes 未満で、
queue や入力長とともに保持される workspace はありません。

更新した default/1/5/10/20 Exact/IPP-fast 60-run matrix は、各 cell の各 captured surface で
1 hash を維持しました。focused DSP 46 tests は native Intrinsics 有効時と全 hardware Intrinsics
無効時の両方で成功し、standard xUnit v3 suite は 1,607 pass、3 environment skip でした。

### current VHS inverse scheduling の再配分

新しい 320-frame Exact `current --threads 20` CPU trace では、active RF-block work が
約 36.6 CPU 秒、inverse companion lane が約 14.2 CPU 秒でした。従来の 12 outer
workers + 8 dedicated companions では companion lane が軽く空いていました。採用した
policy は、20 workers の要求時に 14 outer workers + 6 companions を使用します。
28-block lookahead と既存の cache、output、workspace bound は変更しません。v0.4.0、
20 workers 未満、GNRC、sharpness は従来の scheduling を維持します。

変更するのは、すでに独立している inverse をどの bounded worker が担当するかだけです。
FFT plan、buffer、arithmetic、exception priority、cross-field state、output、metadata、
diagnostic は変更していません。実行順を反転した 1,000-frame Exact `current` 2 pair は、
luma、chroma、raw JSON、stdout、normalized stderr/log、全 ordered `fileLoc` で一致しました。
combined median は 32.962 から 32.699 秒（wall time 0.80% 減）、CPU time は 305.711
から 296.094 秒（3.15% 減）へ移りました。CPU work が wall time より速く減ったため、
effective cores は 9.27 から 9.06 へ変わりました。peak working set は約 395-397 MiB で
bounded のままです。

interleaved 160-frame IPP-fast `current` 4 pair も同じ 7 surface で一致し、median wall
time は 8.752 から 8.584 秒（1.92% 減）へ移りました。CPU time は実質的に中立で、
effective cores は 5.85 から 5.95 へ増えました。別の v0.4.0 Exact/IPP-fast pair は中立です。
更新した default/1/5/10/20 の 60-run matrix は、全 cell の全 surface で 1 hash を維持しました。
Release test は native intrinsics で 1,607 passed/3 environment skips、AVX2 disabled で
1,607/3、all hardware intrinsics disabled で 1,606/4 でした。

### Exact high-worker sync preprocessing

採用した candidate は、8 workers 以上を要求した Exact `current` path に限り VHS boxcar/sync
preprocessing の上限を 4 から 8 にします。default 5-worker と全 IPP-fast path は従来の
4-worker 上限を維持します。DSP expression、演算順序、field state、ordered commit、
metadata、diagnostic は変更していません。

standalone sync-detector A/B は median wall time を 651.775 から 404.724 ms（1.610x）へ
短縮し、output は一致しました。interleaved 160-frame Exact `current --threads 20` 6 pair
では median wall time が約 2.4% 短縮し、1,000-frame pair は 33.760 から 32.946 秒へ
移りました（wall time 2.41% 減、throughput 1.0247x、2.47% 増）。effective core use は
8.62 から 9.16 へ増加しました。luma、
chroma、raw JSON、stdout、normalized stderr/log、ordered `fileLoc` は一致し、peak
working set の増加は約 2.5 MiB で bounded でした。IPP-fast まで cap 8 にする初期 variant
は 3.2% 遅かったため削除し、IPP-fast は cap 4 のままです。

final 60-run matrix は Exact/IPP-fast、v0.4.0/`current`、default、1、5、10、20 workers を
各 cell 3 run で網羅し、7 captured surface は各 cell で 1 hash だけでした。Release build は
warning/error なしです。pinned IPP runtime を stage した standard xUnit v3 suite は 1,609 tests
を discover し、1,606 passed、CUDA/AMF environment 不在による expected skip は 3 でした。
...
最後の 3 形式は独立した numerical contract の optional backend を明示的に選択します。
