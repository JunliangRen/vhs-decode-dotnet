...
現在の .NET port release は `v0.4.0-2.9.0`（application version `2.9.0`）です。
...
全 60 回の .NET 測定は 2026-08-26 に main commit `eb7ba6e` と staged VHS payload-prefix
materialization を基にした同じ self-contained .NET 11 Preview 7 candidate でまとめて更新しました。
...
更新した全 .NET cell は main commit `eb7ba6e` と staged VHS payload-prefix
materialization を基にした self-contained Preview 7 candidate を使いました。product version は
`2.9.0+eb7ba6efc6c894a85720d4e26970d89982f66d10`、137,280,308-byte single-file
`decode.exe` の SHA-256 は
`6C75525B3F5FCFE428BE5CCFC9CD787FE1CFC40EB4DF14263BDC2685ABD58640` です。
...
envelope と payload materialization を維持します。final 60-run matrix は 5 worker setting の
...
compatibility surface が一致しました。1,000-frame の combined baseline/candidate wall time は
59.306/58.570 秒（1.24% lower）、CPU time は 558.547/552.453 秒（1.09% lower）でした。
2 candidate memory trace は progressive growth を示しませんでしたが、1 run は高く平坦な
507.9 MiB peak だったため、memory reduction ではなく bounded memory のみを主張します。
