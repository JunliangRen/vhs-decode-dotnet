...
public sealed class PocketFftMixedRadixCompatibilityTests
{
    ...
    [Fact(DisplayName = "AVX radix-5 stages preserve scalar special-value bits")]
    public void AvxRadix5StagesPreserveScalarSpecialValueBits()
    {
        const int Length = 55;
        uint[][] patternSets =
        [
        ...
        foreach (uint[] patterns in patternSets)
        {
            ...
            Complex32[] actualForward = PocketFftComplex32.ForwardAnyLengthDucc(input);
            Assert.True(
                MemoryMarshal.AsBytes(expectedForward.AsSpan())
                    .SequenceEqual(MemoryMarshal.AsBytes(actualForward.AsSpan())));

            Complex32[] expectedBackward = BackwardDirectScalarReference(expectedForward);
            Complex32[] actualBackward = PocketFftComplex32.BackwardAnyLengthDucc(actualForward);
            Assert.True(
                MemoryMarshal.AsBytes(expectedBackward.AsSpan())
        ...
        ReadOnlySpan<Complex32> input)
    {
        ...
        Complex32[] transformed =
            PocketFftComplex32.TransformDirectScalarReference(conjugated);
        for (int index = 0; index < transformed.Length; index++)
        {
            float imaginary = transformed[index].Imaginary;
            transformed[index] = new Complex32(
