...
- Visual Studio 2026 の `.slnx` には **1,397** 件の標準 xUnit v3 test があり、
...
160-frame snapshot です。source filename は公開しません。2 つの IPP-fast 列を、この
candidate 上で順序を入れ替えた 3 回の Release pass により更新しました。candidate は
merged main `4b9332b` を基にしています。変更のない 2 つの Exact 列と 30 回の Python
reference run は、同じ host と fixture で行った直前の direct refresh を再利用しています。
測定した production source blob は詳細版に固定記録しています。互換性と速度は別々に
評価します。
...
IPP-fast は full-length VHS RF envelope SOS を bounded native context pool で処理するように
なりました。順序を反転した 1,000-frame pair では、v0.4.0 profile の wall time は neutral、
`current` は 0.69% 高速で、process CPU time は 2.8% 減少しました。1,000-frame counter
run の peak は 390.8 MiB、final-third median は first third より 0.8 MiB 高いだけでした。
更新した 30 matrix run と 12-run cross-thread gate は、全 captured surface で deterministic
でした。
