...
- Visual Studio 2026 `.slnx` 包含 **1,442** 项标准 xUnit v3 测试；测试可在
...
2026-08-12 的 30 次固定 Python 参考测量，并用基于 main `0b99402` 的 Phase 24
...
最新输入优化为并行 libsndfile 路径加入懒分配、有界的 2 MiB PCM16 回退环。
重叠 RF block 会复制缓存前缀，只让 libsndfile 解码新尾部；普通读取、映射重启、
真实 seek、fallback、dispose 以及原 PCM16-to-double 表达式的语义均保持不变。
Exact `current` 与 IPP-fast `current` 共四组相反顺序的 1000 帧配对在全部产物与
诊断面上严格一致，平均墙钟分别降低 1.00% 和 0.85%，平均 CPU 时间分别降低
1.28% 和 0.27%。候选工作集保持在 353.0-357.5 MiB。另一个 12 次门禁确认
v0.4.0/current 在 `--threads 0`、默认五 worker 和 `--threads 20` 下均保持确定；
全部 1442 项 xUnit v3 测试通过，完整证据与边界见详细说明。
