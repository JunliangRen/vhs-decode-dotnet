...
- Visual Studio 2026 `.slnx` 包含 **1,421** 项标准 xUnit v3 测试；测试可在
...
Python 与 .NET Release 运行都在本候选上按正序、反序和混排三轮交错测量；候选基于
已合并的 main `a059580`。详细说明中固定记录了被测二进制和三次运行范围；兼容性
结论与速度数据分开判断。

<!-- LATEST_PERFORMANCE_BEGIN -->
| CLI 模式（workers） | Python v0.4.0 | Python PR341 | Exact + v0.4.0 | Exact + current | IPP-fast + v0.4.0 | IPP-fast + current |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 45.605 s | 45.124 s | 12.838 s / 3.552x | 12.749 s / 3.539x | 11.419 s / 3.994x | 9.518 s / 4.741x |
...
所有列都来自同一批交错测量，但倍数仍会随作为分母的 Python 用时变化；使用其他格式
或夹具的历史表格也不能直接横向比较。判断 .NET 是否回退时，应看同夹具、同范围的
.NET 版本 A/B，而不是跨表比较旧倍率。

Exact float32 PocketFFT 第二阶段现在直接原地变换已经连续且归本 worker 所有的 packet，
移除了临时 packet 的分配或租借以及整块复制；因子分解、根表、算术和 scatter 顺序均不变。
6 组交错的 160 帧 `current --threads 20` 配对中，墙钟中位数从 11.59 降至
11.27 秒（2.8%），吞吐提升 2.8%，CPU 时间下降 1.9%；候选赢得 5 组，九类兼容
表面全部一致。4 组单 worker 配对提升 0.8%，CPU 和峰值内存持平。3 组 1000 帧
配对受调度噪声影响，吞吐归类为持平（配对中位数 +0.14%）；峰值工作集保持有界，
约 443 MiB。
