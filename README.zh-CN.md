...
最新一轮 Exact 优化只预计算 PocketFFT radix-8 零频 butterfly 的整数输入/输出
地址；全部浮点表达式和带 twiddle 的循环保持不变。六组反序 160 帧配对在七项
兼容表面上全部一致，进程 CPU 中位数下降 3.5%，墙钟保持中性。两组相反顺序的
1000 帧配对把合计 CPU 时间从 1,056.06 秒降至 1,017.89 秒（下降 3.6%），
合计墙钟从 138.96 秒降至 138.31 秒（下降 0.47%），分配保持中性且没有渐进
减速。由于尚未形成新的稳定整条流水线加速档位，上表保持不变。
