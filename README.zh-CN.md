...
2026-08-20 的本机资源矩阵使用源提交 `42674ca`、同一份真实 40 MSPS PAL 采集、
Intel Core Ultra 7 265K（20 个逻辑处理器）与 RTX 4070。每行是两次独立启动进程的
平均值，括号内为两次实测源帧率范围。

| 路径 | 源 fps（范围） | `decode.exe` CPU | 整机 CPU | GPU SM 平均/峰值 | NVENC 平均/峰值 | GPU 显存峰值 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 完整 CUDA 40 MSPS | 62.90（62.36-63.43） | 13.62% / 2.72 核 | 38.84% | 37.01% / 65% | 0% / 0% | 7,076 MiB |
| 完整 IPP 40 MSPS | 24.74（24.67-24.81） | 23.16% / 4.63 核 | 29.00% | 0.08% / 3% | 0% / 0% | 3,141 MiB |
| 完整 CUDA 20 MSPS | 69.72（69.51-69.93） | 14.13% / 2.83 核 | 41.42% | 27.38% / 58% | 0% / 0% | 5,328 MiB |
| 完整 IPP 20 MSPS | 32.89（32.60-33.17） | 27.28% / 5.46 核 | 44.24% | 0.04% / 1% | 0% / 0% | 3,141 MiB |
| 预览 CUDA 20 MSPS | 84.19（83.04-85.33） | 5.52% / 1.10 核 | 30.36% | 41.99% / 63% | 5.70% / 14% | 4,835 MiB |
| 预览 IPP 20 MSPS | 33.00（32.91-33.09） | 23.44% / 4.69 核 | 36.15% | 1.25% / 11% | 0.48% / 11% | 3,323 MiB |
...
驱动、采样器与其他系统负载。GPU 数据来自全局 100 ms NVML 采样。紧邻测试前的空闲
基线为整机 CPU 7.32%、GPU SM 0.32%、NVENC 0%、显存 3,141 MiB。CUDA 在完整
40、完整 20 与预览 20 中的源帧吞吐分别为 IPP 的 2.54x、2.12x 与 2.55x。完整方法见
[详细性能说明](docs/README.detailed.zh-CN.md#当前-ippcuda-资源矩阵)。

此前的五窗口渲染画质对比与本次吞吐门禁分开保留。计入一场时间偏移后，平均 SSIM
Y/U/V/All 为 0.916844/0.957443/0.966783/0.931934；相对采用相同同步策略但未启用新
dropout/色度处理的 CUDA 输出，五个窗口全部改善。另一组 A-B-B-A 测得 CUDA 墙钟
代价为 1.2%。这些数字只适用于该采集与硬件，不代表与 Exact 等价。
...
门禁中，20 MSPS 让 CUDA 吞吐提高 10.85%、IPP 提高 32.94%。较早的 100 帧短门禁
...
采集上，修正画质后的 FP32 CUDA-full 路径在观感上已明显接近 Exact。上面的最新矩阵
取代了优化前的旧速度对比：40 MSPS 下 CUDA 为 62.90 源 fps、IPP 为 24.74（2.54x）；
20 MSPS 下为 69.72 对 32.89（2.12x）。每个变体各自两次生成的亮度、色度和 JSON
