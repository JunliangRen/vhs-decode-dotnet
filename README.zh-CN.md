...
- Visual Studio 2026 `.slnx` 包含 **1,446** 项标准 xUnit v3 测试；测试可在
...
本次改动影响的 45 次 .NET 测量。Exact `current` 的全部 15 次运行已在
2026-08-14 用基于 main `c47cffc` 的最新候选刷新。每个单元格均有三次完整运行，
兼容性结论与速度数据分开判断。

<!-- LATEST_PERFORMANCE_BEGIN -->
| CLI 模式（workers） | Python v0.4.0 | Python PR341 | Exact + v0.4.0 | Exact + current | IPP-fast + v0.4.0 | IPP-fast + current |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 52.811 s | 54.243 s | 12.095 s / 4.366x | 12.427 s / 4.365x | 10.797 s / 4.891x | 9.222 s / 5.882x |
...
最新 Exact `current` 优化会在大型托管 real-FFT inverse 中并行准备彼此独立的共轭
配对。它保留相同的 float32 值、twiddle 查询、表达式顺序、中心 bin 覆盖、FFT 和
有界 worker 私有 workspace。三组交错的 1000 帧 release 二进制配对在全部采集面
严格一致，墙钟中位数从 35.822 降至 33.715 秒（5.88%，1.063x）。CPU 时间中位数
从 298.891 降至 284.063 秒（4.96%），有效核心占用从 8.34 增至 8.43；峰值工作集
和峰值 private bytes 中位数分别降低 3.01% 和 3.71%。

含启动开销较高的 160 帧 apphost 矩阵结果有升有降：默认、1、5、10 workers 的候选
中位数分别快 0.08%、0.44%、1.85%、2.59%，20 workers 则慢 4.14%。同一时刻的
三组配对核对中，旧 `bdccd58` 单 worker 二进制为 41.77 秒，当前 main 为 41.04 秒；
因此本次表格相对上一快照的倍数下降来自跨日期主机状态，而不是版本回退。另一个
12 次线程门禁保持确定；1446 项 xUnit v3 套件门禁通过，其中 1442 项通过、0 项失败，
4 项仅 IPP 测试因本地缺少原生运行库而跳过。
