...
| 上游行为配置 | 分阶段实现 | `v0.4.0` 仍是默认值；`current` 是显式启用的配置，前三个已实现阶段是已合并 PR 341 的 VHS HSync 检测器、VSync 电平精修和 NTSC 色度群延迟校正。 |
...
| `current` | 否 | 已合并的 PR 341，commit `2f21e8ed6018b14561396cc95f1f6828054470b8` | exact-first VHS HSync 候选提取、MAD 异常值剔除、多网格锁定、电平校准、亚采样脉冲合成、稳健 VSync 电平精修和 NTSC 色度群延迟校正。 |

`current` 会分阶段推进。Super-Gaussian 最终滤波器和 CTI 仍处于待实现状态，
不会因为选择该配置而被静默启用；嵌入的基线目录会明确记录这一边界。
...
旧路径、实际并行阈值及上游全部为正的 phase-truthiness 行为。HSync 还保留
...
`dotnet test` 和 Visual Studio Test Explorer 暴露 **976** 个可独立发现的测试。
