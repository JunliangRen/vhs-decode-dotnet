...
最新一轮 Exact 优化直接使用 PocketFFT 实际的最终 radix buffer，不再额外复制
整块 buffer。同一份私有样本的两组相反顺序 1000 帧长跑在七项兼容表面上全部
一致；合计进程 CPU 时间从 1,122.17 秒降至 1,111.08 秒（下降 1.0%），合计
墙钟则基本不变，为 151.78/151.65 秒。这是 CPU 和内存带宽优化，不代表新的
整条流水线加速，因此上表保持不变。
