...
三次交错运行中位数；10 和 20 worker 的四个 `current` 单元格在本分支完成 CTI
与 VHS 同步扫描调优后，以十组严格配对运行重新测量。Exact 并行两次同步扫描，
IPP-fast 只保留有收益的初次扫描。兼容性结论与速度数据分开判断。

<!-- LATEST_PERFORMANCE_BEGIN -->
| CLI 模式（workers） | Python v0.4.0 | Python PR341 | Exact + v0.4.0 | Exact + current | IPP-fast + v0.4.0 | IPP-fast + current |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| 默认（5） | 15.207 s | 16.780 s | 4.389 s / 3.465x | 5.896 s / 2.846x | 3.609 s / 4.213x | 4.654 s / 3.606x |
...
组合各重复三次；四个刷新单元格另增加 80 次严格配对的基线/候选运行。
