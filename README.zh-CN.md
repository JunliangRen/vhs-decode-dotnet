...
标量比较和脉冲顺序均保持不变。以 main `4a67ae9` 为基线，在 `xxf.lds`
（`--start_fileloc 620000000 -l 160`）上，两组交错的默认 worker 配对将平均墙钟/CPU
...
逐位比较。以 `c51f059` 为基线，在同一 `xxf.lds` 160-frame 窗口上，两组交错的默认
worker 配对将平均墙钟/CPU 从 14.060/40.164 秒变为 13.598/40.438 秒（墙钟减少 3.3%；
