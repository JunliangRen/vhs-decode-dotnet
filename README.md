...
| Upstream behavior profiles | Staged | `v0.4.0` remains the default; `current` is an opt-in profile whose first implemented stage is the merged PR 341 VHS HSync detector. |
...
| `current` | No | merged PR 341, commit `2f21e8ed6018b14561396cc95f1f6828054470b8` | Exact-first VHS HSync candidate extraction, MAD rejection, multi-grid lock, calibrated levels, and subpixel pulse synthesis. |

`current` is deliberately staged. VSync level refinement, chroma group-delay
correction, the Super-Gaussian final filter, and CTI are still pending and are
not silently enabled by this profile. The embedded baseline catalog records
that boundary explicitly.

The HSync stage is checked against the original upstream function with
synthetic and upstream PAL fixtures, including pulse coordinates and
floating-point bit patterns. Separate end-to-end gates keep the default
`v0.4.0` profile identical across `--threads 0`, default, and `--threads 20`.
...
project exposes **950** independently discoverable tests to both
