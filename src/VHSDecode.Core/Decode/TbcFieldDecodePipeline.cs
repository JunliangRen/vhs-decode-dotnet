    ...
    bool LaserDiscAgcAdjusted = false)
{
    ...
    internal void AttachOutputBuffers(
        TbcFieldOutputBufferPool.TbcFieldOutputBufferLease outputBufferLease)
        => TbcDecodedFieldOutputBufferRegistry.Attach(this, outputBufferLease);

    internal void RetainOutputBuffers()
        => TbcDecodedFieldOutputBufferRegistry.Retain(this);

    internal void ReleaseOutputBuffers()
        => TbcDecodedFieldOutputBufferRegistry.Release(this);
}
...
internal sealed record TbcFieldDecodeState(
    ...
    VideoOutputConverter CurrentVhsVideoOutput,
...
public sealed class TbcFieldDecodePipeline : IDisposable
{
    ...
    private VideoOutputConverter _currentVhsVideoOutput;
        ...
        DspBackend dspBackend = DspBackend.Exact)
    {
        ...
        _currentVhsVideoOutput = videoOutput;
        ...
        _fieldOutputBufferPool = string.Equals(decodeType, "vhs", StringComparison.Ordinal)
            ? new TbcFieldOutputBufferPool(
                renderer.FrameSpec.FieldSampleCount,
                chromaFieldOptions is null
                    ? null
                    : checked(chromaFieldOptions.OutputLineLength * chromaFieldOptions.OutputLineCount))
            : null;
        ...
        {
            double activeVideoStart = activeVideoStartUs
                ?? throw new ArgumentNullException(nameof(activeVideoStartUs));
            _vhsSyncDetector = new VhsSyncDetector(
    ...
    internal void RestoreStateForRetry(TbcFieldDecodeState state)
    {
        ...
        _currentVhsVideoOutput = state.CurrentVhsVideoOutput;
        ...
        _previousVhsFieldNumber = state.PreviousVhsFieldNumber;
        _previousVhsFieldReadLocation = state.PreviousVhsFieldReadLocation;
    }
    ...
    public static TbcFieldDecodePipeline FromSession(DecodeSession session)
    {
        return new TbcFieldDecodePipeline(
            ...
            workerThreads: session.ExecutionOptions.WorkerThreads,
            upstreamBehaviorProfile: session.ExecutionOptions.UpstreamBehaviorProfile,
            activeVideoStartUs: session.Spec.Name == "vhs"
                && session.ExecutionOptions.UpstreamBehaviorProfile == UpstreamBehaviorProfile.Current
    ...
    private TbcDecodedField Decode(
        ...
        bool usePooledOutputBuffers = false)
    {
        ...
        try
        {
            TbcDecodedField decoded = DecodeCore(
                span,
                syncThresholdHz,
    ...
    internal static int ResolveVhsFieldNumber(
        int? previousFieldNumber,
        long? previousReadLocation,
        long readLocation)
    {
        if (!previousFieldNumber.HasValue || !previousReadLocation.HasValue)
        {
            return 0;
        }

        return readLocation > previousReadLocation.Value
            ? checked(previousFieldNumber.Value + 1)
            : previousFieldNumber.Value;
    }
        ...
        bool deferVhsTail)
    {
        ...
        IReadOnlyList<Pulse> fallbackRawPulses = rawPulses;
        IReadOnlyList<ClassifiedSyncPulse> refinedPulses;
        if (isVhs
            && (_vhsSyncDetector is null || prepared.ExplicitThreshold))
        {
            ...
            fallbackRawPulses = updatedRawPulses;
        }
        else
        {
        ...
        lineLocations = RefineLineLocationsFromHSync(
            span,
            lineLocations,
            activeVideoOutput,
            threshold);
        ...
        if (CanRefineLineLocationsFromBurst(chromaPhase))
        {
            double[] refinedLocations = VhsChromaDecoder.RefineLineLocationsFromBurst(
                ...
                useCurrentFrequencyDrift:
                    _upstreamBehaviorProfile == UpstreamBehaviorProfile.Current,
                fscHz: _chromaFieldOptions.FscMHz * 1_000_000.0);
    ...
    internal VideoOutputConverter ApplyCurrentVhsLevels(
        VhsSyncDetectionResult detection,
        VideoOutputConverter current)
    {
        Ire0AdjustOptions? adjustment = _renderer.Ire0Adjust;
        if (adjustment is null || detection.Pulses.Count == 0)
        {
            return current;
        }

        double ire0 = adjustment.BackPorch
            ? detection.BlankLevel
            : current.Ire0;
        ...
        bool fallbackToSavedLevels = true)
    {
        ...
        if (_vhsSyncDetector is not null)
        {
            bool usedSavedLevels = allowSavedLevels
                && _syncDetectionOptions.UseSavedLevels
                && !_vhsLineLocationIssues;
            return new SyncPreparedSpan(
                span,
                defaultThreshold,
                UsedSavedLevels: usedSavedLevels,
                ConverterOverride: _currentVhsVideoOutput);
        }
        ...
        if (_vsyncSerrationDetector is not null)
        {
            ...
            if (serration.Diagnostic == VsyncSerrationDiagnostic.UnexpectedArbitrage)
            {
                _diagnosticLogger?.Invoke("WARNING", "Unexpected vsync arbitrage");
            }
            else if (serration.Diagnostic == VsyncSerrationDiagnostic.UnexpectedVideoEnvelope)
            {
                _diagnosticLogger?.Invoke("WARNING", "Unexpected video envelope");
            }
                ...
                && serrationBlankLevel.HasValue)
            {
                SerrationLevelRefinement? refined = LevelDetection.RefineSerrationLevels(
                    ...
                    pulseEqualizingLength:
                        _vsyncSerrationDetector.OriginalRateEqualizingPulseLength,
                    pulseLineLength:
                        _vsyncSerrationDetector.OriginalRateLineLength);
                ...
                {
                    bool refinementFallbackHandled = TryResolveVhsSerrationRefinementFallback(
                        refinementFailure,
                        _vhsFieldLevelState,
                        _diagnosticLogger,
                        out (double SyncLevel, double BlankLevel)? refinementFieldStateLevels);
                    if (refinementFieldStateLevels.HasValue)
                    {
                        _vsyncSerrationDetector.PushLevels(
                            refinementFieldStateLevels.Value.SyncLevel,
                            refinementFieldStateLevels.Value.BlankLevel);
                    }
                    else
                    {
                        if (!refinementFallbackHandled
                ...
                && serrationBlankLevel.HasValue)
            {
                ...
                if (serration.FoundSerration)
                {
                    ...
                    if (_upstreamBehaviorProfile == UpstreamBehaviorProfile.V040
                        && _vsyncSerrationDetector.HasLevels
                        && _vhsFieldLevelState is not { HasLevels: true })
                    {
                        _diagnosticLogger?.Invoke(
                            "DEBUG",
                        ...
                        return PrepareSyncSpanFromLevels(
                            span,
                            (_videoOutput.Ire0, referenceSyncLevel),
                            usedSavedLevels: false);
                    }

                    return new SyncPreparedSpan(span, defaultThreshold);
                }
            }
        }
        ...
        if (string.Equals(_decodeType, "vhs", StringComparison.Ordinal))
        {
            ...
            if (pushedSecondPassLevels)
            {
                (double SyncLevel, double BlankLevel)? selectedLevels =
                    SelectVhsLevelsAfterRefinement(
                    syncReference,
                    referenceSyncLevel,
                    _videoOutput.HzIre,
            ...
            (double SyncLevel, double BlankLevel)? terminalLevels =
                SelectTerminalVhsLevels(
                    _vhsFieldLevelState,
                    _lastDetectedSyncLevels);
            if (fallbackToSavedLevels && terminalLevels.HasValue)
            {
                _lastDetectedSyncLevels = terminalLevels;
                return PrepareSyncSpanFromLevels(
                    span,
                    terminalLevels.Value,
                    usedSavedLevels: true);
            }
    ...
    internal static (double SyncLevel, double BlankLevel)? SelectTerminalVhsLevels(
        VhsFieldLevelState? fieldState,
        (double SyncLevel, double BlankLevel)? savedLevels)
    {
        if (fieldState is { HasLevels: true }
            && fieldState.PullLevels() is { } fieldStateLevels)
        {
            return fieldStateLevels;
        }
    ...
    internal static (double SyncLevel, double BlankLevel)? SelectVhsLevelsAfterRefinement(
        ReadOnlySpan<double> syncReference,
        double referenceSyncLevel,
        double hzIre,
        VsyncSerrationDetector? detector,
        ...
        Action<string, string>? diagnosticLogger)
    {
        if (detector is { HasLevels: true })
        {
            return SelectUsableVhsLevels(
                syncReference,
                referenceSyncLevel,
                detector.PullLevels(),
                hzIre,
