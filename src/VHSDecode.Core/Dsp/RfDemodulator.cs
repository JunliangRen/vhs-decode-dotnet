...
public sealed class RfDemodulator : IDisposable
{
        ...
        Complex[]? ownedRfMtfFilter = null)
    {
        ...
        using VhsRealFftWorkspaceLease? vhsRealFftWorkspaceLease = useVhsRealFft
        ...
        else if (useVhsRealRfPath)
        {
            ...
            if (useNumpyComplexVhsAnalytic
                ...
                && _vhsInverseCompanionScheduler?.Value is { } analyticScheduler
                && analyticScheduler.TryQueue(
                    () => BuildNumpyVhsComplexAnalyticSignal(
                        ownedInput,
                        ownedRfVideoFilter,
                        ownedRfMtfFilter,
                        workspace),
                    out VhsInverseCompanionWorkItem analyticWorkItem))
            {
                stagedNumpyAnalytic = analyticWorkItem;
            }
            ...
            {
                DrainSpeculativeVhsAnalytic(stagedNumpyAnalytic);
                throw;
            }
                ...
                vhsRealFftWorkspace!))
        {
            ...
            vhsRealFftWorkspace!.Inverse(boostedHalf, boostedReal);
            BuildVhsAnalyticImaginary(
                boostedHalf,
                input.Length,
                vhsRealFftWorkspace!);
        ...
        NonlinearDeemphasisOptions options)
    {
        ...
        return ApplyNonlinearDeemphasisCore(video, videoSpectrum, options, highPass);
    }

    private static double[] ApplyNonlinearDeemphasisCore(
        ReadOnlySpan<double> video,
        ReadOnlySpan<Complex> videoSpectrum,
        NonlinearDeemphasisOptions options,
        ...
        SubDeemphasisOptions options)
    {
        ...
        Complex[] highPass = BuildNonlinearHighPassFilter(
            ...
            options.Order);
        return ApplySubDeemphasisCore(video, videoSpectrum, sampleRateHz, options, highPass);
    }

    private static double[] ApplySubDeemphasisCore(
        ReadOnlySpan<double> video,
        ReadOnlySpan<Complex> videoSpectrum,
        double sampleRateHz,
        SubDeemphasisOptions options,
    ...
    {
        var output = new Complex[spectrum.Length];
        ApplyNumpyRealFrequencyFilter(spectrum, filter, realLength, output);
        return output;
    }

    private static void ApplyNumpyRealFrequencyFilter(
        ReadOnlySpan<Complex> spectrum,
        ReadOnlySpan<Complex> filter,
        int realLength,
        Span<Complex> output,
        VhsRealFftWorkspace? workspace = null)
    {
        ...
        {
            throw new ArgumentException(
                "Output length must match the real half-spectrum length.",
                nameof(output));
        }

        if (filter.IsEmpty)
        {
            spectrum.CopyTo(output);
            return;
        }
        ...
        {
            workspace.Multiply(spectrum, filter[..spectrum.Length], output);
        }
    }
        ...
        VhsRealFftWorkspace workspace)
    {
        ...
        {
            throw new ArgumentException("VHS FFT workspace length does not match the RF block.", nameof(workspace));
        }

        Span<Complex> hilbertHalfSpectrum = workspace.HilbertHalf.AsSpan(0, filteredHalfSpectrum.Length);
        hilbertHalfSpectrum[0] = Complex.Zero;
    ...
    private static void CompleteParallelVhsInverseStaging(
        VhsRealFftWorkspace workspace,
        Complex[] rfFilteredHalf,
        int spectrumLength,
        VhsInverseCompanionWorkItem analyticTransform)
    {
        try
        {
        ...
        {
            analyticTransform.Dispose();
        }
    }

    private static void DrainSpeculativeVhsAnalytic(
        VhsInverseCompanionWorkItem? analyticTransform)
    {
        if (analyticTransform is null)
        {
            return;
        }
        ...
        {
            // An earlier primary-path failure retains serial exception priority.
        }
        finally
        {
            analyticTransform.Dispose();
        ...
        Span<double> rawEnvelope)
    {
        ...
        {
            throw new ArgumentException("VHS envelope output length must match the filtered RF input.", nameof(rawEnvelope));
        }
        ...
        int offset)
    {
        ...
        if (offset != 0)
        {
            FrequencyDomainFilter.RollInPlace(output, -offset);
        }
    ...
    private static bool ApplyVhsRfHighBoostIfPresent(
        Span<Complex> rfFilteredHalfSpectrum,
        ...
        VhsRealFftWorkspace workspace)
    {
        ...
        if (workspace.First.Length < rfFilteredHalfSpectrum.Length)
        {
            throw new ArgumentException("RF high-boost spectrum buffer is too short.", nameof(workspace));
        }

        workspace.Forward(highPart, workspace.First);
        ReadOnlySpan<Complex> highPartSpectrum = workspace.First.AsSpan(0, rfFilteredHalfSpectrum.Length);
    ...
    {
        // The VHS producer caps normal block concurrency at eight; retain one extra wave
        // for direct callers without allowing large workspaces to spread per thread.
        ...
        {
            ObjectDisposedException.ThrowIf(Volatile.Read(ref _disposed) != 0, this);
            while (_available.TryPop(out VhsRealFftWorkspace? workspace))
            {
                Interlocked.Decrement(ref _retainedCount);
                if (workspace.RealLength == realLength)
                {
                    return new VhsRealFftWorkspaceLease(this, workspace);
                }
        ...
        public void Return(VhsRealFftWorkspace workspace)
        {
            ...
            while (true)
            {
                int retained = Volatile.Read(ref _retainedCount);
                if (retained >= MaximumRetainedWorkspaces)
                {
                    workspace.Dispose();
                    return;
                }
    ...
    private sealed class VhsRealFftWorkspaceLease : IDisposable
    {
        private VhsRealFftWorkspacePool? _owner;
        private VhsRealFftWorkspace? _workspace;

        public VhsRealFftWorkspaceLease(
            VhsRealFftWorkspacePool owner,
            VhsRealFftWorkspace workspace)
        {
            _owner = owner;
        ...
        public VhsRealFftWorkspace Workspace =>
            _workspace ?? throw new ObjectDisposedException(nameof(VhsRealFftWorkspaceLease));

        public void Dispose()
        {
            VhsRealFftWorkspace? workspace = Interlocked.Exchange(ref _workspace, null);
            VhsRealFftWorkspacePool? owner = Interlocked.Exchange(ref _owner, null);
            if (workspace is not null && owner is not null)
            {
                owner.Return(workspace);
            }
        }
    }
        ...
        bool useVhsRealFft)
    {
        ...
        if (useVhsRealFft)
        {
            ...
            {
                throw new ArgumentException(
                    "Video half-spectrum length does not match video length.",
                    nameof(videoSpectrum));
            }
        ...
        else
        {
            ...
            {
                throw new ArgumentException(
                    "Video spectrum length must match video length.",
                    nameof(videoSpectrum));
            }
        ...
        NonlinearDeemphasisOptions options)
    {
        ...
        Complex[] highPass = BuildNonlinearHighPassFilter(sampleRateHz, video.Length, options);
        return ApplyNonlinearDeemphasisRealCore(video, videoSpectrum, options, highPass);
    }

    private static double[] ApplyNonlinearDeemphasisRealCore(
        ReadOnlySpan<double> video,
        ReadOnlySpan<Complex> videoSpectrum,
        NonlinearDeemphasisOptions options,
        ReadOnlySpan<Complex> highPass)
        ...
        SubDeemphasisOptions options)
    {
        ...
        Complex[] highPass = BuildNonlinearHighPassFilter(
            ...
            options.BandPassUpperHz,
            options.Order);
        return ApplySubDeemphasisRealCore(video, videoSpectrum, sampleRateHz, options, highPass);
    }

    private static double[] ApplySubDeemphasisRealCore(
        ReadOnlySpan<double> video,
        ReadOnlySpan<Complex> videoSpectrum,
        double sampleRateHz,
        SubDeemphasisOptions options,
        ...
        NonlinearDeemphasisOptions options)
    {
        ...
        lock (_nonlinearDeemphasisFilterPlanLock)
        {
            ...
            plan = new DeemphasisFilterPlan<NonlinearDeemphasisOptions>(
                length,
                options,
                BuildNonlinearHighPassFilter(SampleRateHz, length, options));
            Volatile.Write(ref _nonlinearDeemphasisFilterPlan, plan);
            _nonlinearDeemphasisFilterPlanBuildCount++;
            return plan.HighPass;
        }
    }

    private Complex[] GetSubDeemphasisFilter(
        int length,
        SubDeemphasisOptions options)
    {
        ...
        lock (_subDeemphasisFilterPlanLock)
        {
            ...
            plan = new DeemphasisFilterPlan<SubDeemphasisFilterKey>(
                ...
                BuildNonlinearHighPassFilter(
                    ...
                    options.BandPassUpperHz,
                    options.Order));
            Volatile.Write(ref _subDeemphasisFilterPlan, plan);
            _subDeemphasisFilterPlanBuildCount++;
            return plan.HighPass;
        }
    }

    private void ValidateSubDeemphasisOptions(SubDeemphasisOptions options)
    {
        if (options.Deviation <= 0.0)
        {
            throw new ArgumentOutOfRangeException(
                nameof(options),
                "Sub-deemphasis deviation must be positive.");
        }
